...
class HLLaundryState(object):
    ...
    def __init__(self):
        ...
        self.goal = "(:goal (and (ClothInWasher cloth washer) (not (WasherDoorOpen washer)) (not (clothInRegion1 cloth)) (not (clothInRegion2 cloth)) (not (clothInRegion3 cloth)) (not (clothInRegion4 cloth))))\n"
...
class LaundryEnvironmentMonitor(object):
    ...
    def predict_basket_location(self):
        ...
        act_num += 1
        ll_plan_str.append('{0}: MOVETO BAXTER ARMS_FORWARD_{1} BASKET_SCAN_POSE_{2} \n'.format(act_num, self.state.robot_region, self.state.robot_region))
        act_num += 1
        ll_plan_str.append('{0}: ROTATE BAXTER BASKET_SCAN_POSE_{1} BASKET_SCAN_POSE_2 REGION2 \n'.format(act_num, self.state.robot_region))
        act_num += 1
    ...
    def update_plan(self, plan, cloth_to_region={}, in_washer=False, reset_to_region=-1, cloth_z=0.625):
        ...
        while len(self.state.region_poses[1]):
            ...
            for offset in utils.wrist_im_offsets:
                ...
                if cloth_present:
                    ...
                    accept = len(plan.params['baxter'].openrave_body.get_ik_from_pose(np.r_[cloth_pos[:2], 0.7]+np.r_[offset[1], 0], [0, np.pi/2, 0], "left_arm")) > 0
                    if accept:
                        final_offset = offset
                        break

            cloth_pos[:2] += np.array(final_offset[1])
            ...
            act_num += 1
            ll_plan_str.append('{0}: ROTATE_HOLDING_CLOTH BAXTER CLOTH0 LOAD_BASKET_INTER_2 ROBOT_REGION_3_POSE_1 REGION3 \n'.format(act_num))
            act_num += 1
    ...
    def load_basket_from_region_3(self):
        ...
        while len(self.state.region_poses[2]):
            ...
            ll_plan_str.append('{0}: PUT_INTO_BASKET BAXTER CLOTH0 BASKET CLOTH_TARGET_END_0 BASKET_FAR_TARGET CLOTH_PUTDOWN_BEGIN_0 CP_EE_0 CLOTH_PUTDOWN_END_0 \n'.format(act_num))
            act_num += 1
            ll_plan_str.append('{0}: MOVETO BAXTER CLOTH_PUTDOWN_END_0 LOAD_BASKET_FAR \n'.format(act_num))
            act_num += 1
    ...
    def load_basket_from_region_4(self):
        ...
        while len(self.state.region_poses[1]):
            ...
            final_offset = ([0,0], [0,0])
            for offset in utils.wrist_im_offsets:
                cloth_present = self.cloth_predictor.predict_wrist_center(offset[3])
                if cloth_present:
    ...
    def load_washer_from_basket(self):
        ...
        while len(self.state.region_poses[4]):
            ...
            final_offset = ([0,0], [0,0])
            for offset in utils.wrist_im_offsets:
                cloth_present = self.cloth_predictor.predict_wrist_center(offset[1])
                if cloth_present:
    ...
    def load_washer_from_region_1(self):
        ...
        while len(self.state.region_poses[0]):
            ...
            ll_plan_str.append('{0}: CLOTH_GRASP BAXTER CLOTH0 CLOTH_TARGET_BEGIN_0 {1} CG_EE_0 CLOTH_GRASP_END_0 \n'.format(act_num, last_pose))
            act_num += 1
            ll_plan_str.append('{0}: MOVEHOLDING_CLOTH BAXTER CLOTH_GRASP_END_0 LOAD_WASHER_INTERMEDIATE_POSE CLOTH0 \n'.format(act_num))
            act_num += 1
            ll_plan_str.append('{0}: MOVEHOLDING_CLOTH BAXTER LOAD_WASHER_INTERMEDIATE_POSE PUT_INTO_WASHER_BEGIN CLOTH0 \n'.format(act_num))
    ...
    def reset_laundry(self):
        ...
        while len(self.state.region_poses[6]):
            ...
            if next_region != 3:
                ll_plan_str.append('{0}: ROTATE_HOLDING_CLOTH BAXTER CLOTH0 LOAD_BASKET_FAR ROBOT_REGION_{1}_POSE_1 REGION{2}'.format(act_num, next_region, next_region))
                act_num += 1
