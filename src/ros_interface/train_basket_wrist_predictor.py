...
class image_converter:
    ...
    def collect_images(self):
        label = [0.75, 0.02, 0.83-const.ROTOR_BASE_HEIGHT, np.pi/2] # Centered on table  & neutral rotation
        ...
        left_arm_pose = self.robot.openrave_body.get_ik_from_pose([label[0] - np.cos(label[3]-np.pi)*handle_offset, label[1] + np.sin(label[3])*handle_offset, label[2]+.05], [0, np.pi/2, 0], "left_arm")
        ...
        if len(left_arm_pose):
            self.left.move_to_joint_positions(get_joint_positions('left', left_arm_pose[0].reshape((7,1)), 0))
            terminate = input("Did the left arm grasp the basket without moving it? ")
        if terminate:
                ...
                for k in range(17):
                    ...
                    for angle in angles:
                        ...
                        if len(arm_pose):
                            ...
                            for k in left_actual_pos.keys():
                                if abs(left_actual_pos[k] - left_ref_pose[k]) > 1e-2:
                            ...
                            if joints_are_close:
                                ...
                                print 'Skipped image'
                terminate = input('Up to angle {0}. Continue? '.format(angle))
                if terminate:
                    return
