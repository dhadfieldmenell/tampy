...
class RobotLLSolver(LLSolver):
    ...
    def obj_pose_suggester(self, plan, anum, resample_size=20):
        ...
        if act.name == 'cloth_putdown_near' or act.name == 'cloth_putdown_near_right':
            act.params[2].value[:2,0] = act.params[3].value[:2,0] + np.random.uniform(-0.04, 0.04, (2,))
            act.params[2].value[2,0] = act.params[3].value[2,0]
        if next_act != None and (next_act.name == 'cloth_putdown_near' or next_act.name == 'cloth_putdown_near_right'):
        ...
        for i in range(resample_size):
            if act.name == "rotate_holding_basket_with_cloth" or act.name == "rotate_holding_basket":
                ...
                target_pos = target_body.env_body.GetLink('washer_door').GetTransform().dot([-0.15,-0.1,0,1])[:3]

                # old_pose = next_act.params[2].value[:,0]
                # robot_body.set_pose([0, 0, old_pose[0]])

                random_dir = np.multiply(np.random.sample(3) - [0, 1, -1.0], [0.01, 0.05, 0.1])
                ee_pos = target_pos + random_dir
                ...
                target_body.set_pose(target.value[:, 0], target.rotation[:, 0])
                target_body.set_dof({'door': target.door[:, 0]})
                target_pos = target_body.env_body.GetLink('washer_door').GetTransform().dot([-0.25,0.15,0.25,1])[:3]

                # old_pose = next_act.params[2].value[:,0]
                # robot_body.set_pose([0, 0, old_pose[0]])

                random_dir = np.multiply(np.random.sample(3) - [1, 0, -1], [0.2, 0.1, 0.0])
                ee_pos = target_pos + random_dir
                ...
                target_body.set_pose(target.value[:, 0], target.rotation[:, 0])
                target_body.set_dof({'door': target.door[:, 0]})
                target_pos = target_body.env_body.GetLink('washer_door').GetTransform().dot([-0.17,-0.07,0,1])[:3]

                # old_pose = act.params[2].value[:,0]
                # robot_body.set_pose([0, 0, old_pose[0]])

                random_dir = np.multiply(np.random.sample(3) - [0, 1, 0], [0.01, 0.05, 0.1])
                ee_pos = target_pos + random_dir
                ee_rot = np.array([0, np.pi/2, 0])
                ik_arm_poses = robot_body.get_ik_from_pose(ee_pos, ee_rot, "left_arm")
                if not len(ik_arm_poses):
                ...
                target_body.set_pose(target.value[:, 0], target.rotation[:, 0])
                target_body.set_dof({'door': target.door[:, 0]})
                target_pos = target_body.env_body.GetLink('washer_door').GetTransform().dot([-0.3,0.15,0,1])[:3]

                # old_pose = act.params[2].value[:,0]
                # robot_body.set_pose([0, 0, old_pose[0]])

                random_dir = np.multiply(np.random.sample(3) - [2, 1, 0], [0.1, 0.1, 0.05])
                ee_pos = target_pos + random_dir
                ee_rot = np.array([0, np.pi/2, 0])
                ik_arm_poses = robot_body.get_ik_from_pose(ee_pos, ee_rot, "left_arm")
                if not len(ik_arm_poses):
            ...
            elif next_act != None and (next_act.name == 'cloth_putdown_right' or next_act.name == 'cloth_putdown_near_right' or next_act.name == 'cloth_putdown_in_region_right'):
                target = next_act.params[2]
                target_pos = target.value[:, 0]
                # if target pose is not initialized, all entry should be 0
                if np.allclose(target_pos, 0):
                ...
                random_dir = np.multiply(np.random.sample(3) - [0.5,0.5,-1.0], [0.2, 0.2, 0.1])
                ee_pos = target_pos + random_dir
            ...
            elif act.name == 'cloth_grasp' or act.name == 'cloth_putdown' or act.name == 'cloth_putdown_near' or act.name == 'cloth_putdown_in_region_left':
                target = act.params[2]
                target_body = act.params[1].openrave_body
                target_body.set_pose(target.value[:, 0], target.rotation[:, 0])
                target_pos = target.value[:, 0]
            ...
            elif act.name == 'cloth_grasp_right' or act.name == 'cloth_putdown_right' or act.name == 'cloth_putdown_near_right' or act.name == 'cloth_putdown_in_region_right':
                target = act.params[2]
                target_body = act.params[1].openrave_body
                target_body.set_pose(target.value[:, 0], target.rotation[:, 0])
                target_pos = target.value[:, 0]
                ...
                random_dir = np.multiply(np.random.sample(3) - [-2, 3, 0], [.1, .05, .05])
                ee_pos = target_pos + random_dir
                ...
                random_dir = np.multiply(np.random.sample(3) - [0.5,0.5,0.5], [0.005, 0.005, 0.005])
                ee_left = target_pos + random_dir
                ...
                r_arm_pose = robot_body.get_ik_from_pose(ee_right, target_rot_right, "right_arm")
                if not len(l_arm_pose) or not len(r_arm_pose):
                    # import ipdb; ipdb.set_trace()
                    continue
                l_arm_pose = baxter_sampling.closest_arm_pose(l_arm_pose, old_l_arm_pose.flatten()).reshape((7,1))
                ...
                random_dir = np.multiply(np.random.sample(3) - [0.5,0.5,-1.0], [0.01, 0.01, 0.01])
                ee_left = target_pos + random_dir
                ...
                random_dir = np.multiply(np.random.sample(3) - [0.5,0.5, 0.0], [0.005, 0.005, 0.005])
                ee_left = target_pos_left + random_dir
                ...
                random_dir = np.multiply(np.random.sample(3) - [0.5,0.5, 0.5], [0.001, 0.001, 0.001])
                ee_left = target_pos
