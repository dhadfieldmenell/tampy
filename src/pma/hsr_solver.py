    ...
    def obj_pose_suggester(self, plan, anum, resample_size=10):
        ...
        for i in range(resample_size):
            if next_act != None and (next_act.name == 'can_grasp' or next_act.name == 'can_putdown'):
                ...
                robot_pos = None
                attempt = 0
                while robot_pos is None or \
                      collisionChecker.CheckCollision(robot_body.env_body, report=col_report) or \
                      col_report.minDistance <= 0.01:

                    robot_angle = np.random.uniform(angle_offset-np.pi/3, angle_offset+np.pi/3)
