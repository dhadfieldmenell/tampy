...
class CanSolver(backtrack_ll_solver.BacktrackLLSolver):
    ...
    def obj_pose_suggester(self, plan, anum, resample_size=10):
        ...
        for i in range(resample_size):
            if next_act and (next_act.name == 'right_grasp' or next_act.name == 'right_putdown'):
                ...
                rposes = robot.openrave_body.get_ik_from_pose(target_pos, DOWN_ROT, 'rightarm_torso')
                if not len(rposes):
                    pass
            ...
            elif next_act and (next_act.name == 'left_grasp' or next_act.name == 'left_putdown'):
                target = next_act.params[2]
                target_pos = target.pose[:,0] + np.array([0, 0, 0.1])
                next_act.params[1].openrave_body.set_pose(target.pose[:,0], target.rotation[:, 0])
                robot.openrave_body.set_dof({'rArmPose': old_r_arm_pose})
                lposes = robot.openrave_body.get_ik_from_pose(target_pos, DOWN_ROT, 'leftarm_torso')
                if not len(lposes):
                    pass
                ...
                target = act.params[2]
                target_pos = target.pose[:,0] + np.array([0, 0, 0.1])
                act.params[1].openrave_body.set_pose(target.pose[:,0], target.rotation[:, 0])
                robot.openrave_body.set_dof({'rArmPose': old_r_arm_pose})
                rposes = robot.openrave_body.get_ik_from_pose(target_pos, DOWN_ROT, 'rightarm_torso')
                if not len(rposes):
                    pass
