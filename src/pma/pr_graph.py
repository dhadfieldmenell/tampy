...
def p_mod_abs(hl_solver, ll_solver, domain, problem, initial=None, goal=None, suggester = None, max_iter=100, debug = False):
    ...
    n0 = HLSearchNode(hl_solver.translate_problem(problem, initial, goal), domain, problem, priority=0)
    Q = PriorityQueue()
    ...
    for _ in range(max_iter):
        ...
        if n.is_hl_node():
            ...
            if n.gen_child():
                ...
                c = HLSearchNode(hl_solver.translate_problem(n_problem, goal=goal), domain, n_problem, priority=n.priority + 1, prefix=n.curr_plan.prefix(fail_step))
                Q.put((c.heuristic(), c))
