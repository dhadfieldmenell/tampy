...
class BaxterPolicyPredicate(ExprPredicate):
    def __init__(
        ...
        super(BaxterPolicyPredicate, self).__init__(
            name,
            e,
            self.attr_inds,
            [],
        ...
        for p_name in self.plan.params:
            ...
            for attr in const.ATTR_MAP[p._type]:
                if (p.name, attr[0]) in self.action_inds:
                    self._u[self.action_inds[p.name, attr[0]]] = getattr(p, attr[0])[
                        :, t
                    ]
    ...
    def error_grad(self, x):
        ...
        jac[
            self.action_inds[("baxter", "lArmPose")],
            self.action_inds[("baxter", "lArmPose")] + (active_len - 1) * self.dU,
        ] = -1.0
        jac[
            self.action_inds[("baxter", "lGripper")],
            self.action_inds[("baxter", "gripper")] + (active_len - 1) * self.dU,
        ] = -1.0
        jac[
            self.action_inds[("baxter", "rArmPose")],
            self.action_inds[("baxter", "rArmPose")] + (active_len - 1) * self.dU,
        ] = -1.0
        jac[
            self.action_inds[("baxter", "rGripper")],
            self.action_inds[("baxter", "rGripper")] + (active_len - 1) * self.dU,
        ] = -1.0
