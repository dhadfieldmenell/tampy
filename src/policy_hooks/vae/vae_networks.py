...
def conv2d(inputs, n_filters, filter_size, padding, name, reuse, strides=1, var_scale=0.5):
    import tensorflow as tf
...
def dense(inputs, units, name, reuse, bias_initializer=None, var_scale=0.5):
    import tensorflow as tf
    with tf.variable_scope(name, reuse=reuse):
        ...
        with tf.variable_scope('encoder', reuse=reuse):
            if n_channels is not None:
                ...
                for n_c, fs, s, i in zip(n_channels, filter_sizes, strides, n_conv_layers):
                    ...
                    out = tf.layers.batch_normalization(out, training=training, name='batch_encoder{0}'.format(i))
                    out = tf.nn.relu(out)
