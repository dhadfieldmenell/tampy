...
class EnvWrapper:
    ...
    def get_attr(self, obj, attr, euler=False):
        ...
        if attr in self.geom.jnt_names:
            ...
            if attr in self.geom.arms:
                ...
                cv, ov = (
                    self.geom.get_gripper_closed_val(),
                    self.geom.get_gripper_open_val(),
                )
    ...
    def set_item_pose(
        self, item_name, pos=None, quat=None, forward=False, order="xyzw", euler=False
    ):
        if item_name == "sawyer":
            return
        true_name = item_name
        ...
        try:
            ...
            ind = self.env.mjpy_model.body_name2id(item_name)
            if pos is not None:
                self.env.sim.data.body_xpos[ind] = pos
            if quat is not None:
...
class RobotAgent(TAMPAgent):
    def __init__(self, hyperparams):
        ...
        self.ctrl_mode = (
            "joint" if ("sawyer", "right") in self.action_inds else "ee_pos"
        )
        ...
        if self.load_render:
            ...
            self.render_context = MjRenderContextOffscreen(
                self.base_env.sim, device_id=0
            )
    ...
    def get_annotated_image(self, s, t, cam_id=None):
        ...
        for ctxt in self.base_env.sim.render_contexts:
            ...
            ctxt._overlay[mj_const.GRID_BOTTOMLEFT] = [
                "{0: <7} {1: <7} {2}".format(precost, postcost, gripcmd),
                "",
            ]
    ...
    def run_policy_step(self, u, x):
        ...
        if "right_ee_pos" in ctrl:
            ...
            gripper /= 2
            targ_pos = (
                self.mjc_env.get_attr("sawyer", "right_ee_pos") + ctrl["right_ee_pos"]
            )
            ...
            for n in range(n_steps + 1):
                ...
                rot_ctrl = (
                    -sign1
                    * sign2
                    * robo_T.get_orientation_error(sign1 * targrot, sign2 * curquat)
                )
                self.cur_obs, rew, done, _ = self.base_env.step(
                    np.r_[pos_ctrl, rot_ctrl, [gripper]]
                )
        ...
        if "right" in ctrl:
            ...
            for n in range(n_steps + 1):
                ctrl = np.r_[
                    targ_pos - self.mjc_env.get_attr("sawyer", "right"), gripper
                ]
    ...
    def set_symbols(self, plan, task, anum=0, cond=0, targets=None, st=0):
        ...
        for arm in params[0].geom.arms:
            ...
            if hasattr(params[0], rot_ee_attr):
                getattr(params[3], rot_ee_attr)[:, 0] = getattr(params[0], rot_ee_attr)[
                    :, st
                ]
        ...
        for pname in plan.params:
            if "{0}_init_target".format(pname) in plan.params:
                ...
                if hasattr(plan.params[pname], "rotation"):
                    plan.params["{0}_init_target".format(pname)].rotation[
                        :, 0
                    ] = plan.params[pname].rotation[:, st]
    ...
    def fill_sample(
        ...
        if LEFT_GRIPPER_ENUM in self.sensor_dims:
            sample.set(
                LEFT_GRIPPER_ENUM,
                mp_state[self.state_inds["sawyer", "left_gripper"]],
                t,
            )
        if RIGHT_GRIPPER_ENUM in self.sensor_dims:
            sample.set(
                RIGHT_GRIPPER_ENUM,
                mp_state[self.state_inds["sawyer", "right_gripper"]],
                t,
            )
    ...
    def goal_f(
        ...
        if self.goal_type == "moveto":
            ...
            if cont:
                return cost
            return 1.0 if len(preds) else 0.0
        ...
        if self.goal_type == "grasp":
            ...
            if verbose and len(preds):
                print(
                    "FAILED:",
                    preds,
                    preds[0][1].expr.expr.eval(preds[0][1].get_param_vector(T)),
                    self.process_id,
                )
            if cont:
                return cost
    ...
    def reset_to_state(self, x, full=True):
        ...
        for (pname, aname), inds in self.state_inds.items():
            if pname == "table":
                continue
            if aname.find("ee_pos") >= 0 or aname.find("ee_rot") >= 0:
                continue
            val = x[inds]
        ...
        for (pname, aname), inds in self.state_inds.items():
            ...
            if clip:
                if aname in ["left", "right"]:
                    ...
                    cv, ov = (
                        self.sawyer.geom.get_gripper_closed_val(),
                        self.sawyer.geom.get_gripper_open_val(),
                    )
                    val = (
                        ov
                        if np.max(np.abs(val - cv)) > np.max(np.abs(val - ov))
                        else cv
                    )

            if len(inds) != len(val):
                raise Exception(
                    "Bad state retrieval for",
                    pname,
                    aname,
                    "expected",
                    len(inds),
    ...
    def get_random_initial_state_vec(self, config, plans, dX, state_inds, ncond):
        ...
        for ind, obj in enumerate(self.obj_list):
            ...
            if (obj, "pose") in self.state_inds:
                targets[targ] = self.mjc_env.get_item_pose(
                    "Visual{}_main".format(obj.capitalize())
                )[0]
    ...
    def set_to_target(self, obj, targets=None):
        if targets is None:
            targ_val = self.mjc_env.get_item_pose(
                "Visual{}_main".format(obj.capitalize())
            )[0]
            targ_val[2] -= self.mjc_env.z_offsets[obj]
        else:
            targ_val = targets[self.target_inds["{}_end_target".format(obj), "value"]]
        self.mjc_env.set_item_pose(obj, targ_val, [0.0, 0.0, 0.0, 1.0], forward=True)

    def replace_cond(self, cond, curric_step=-1):
        self.cur_obs = self.mjc_env.reset()
        x, targets = self.get_random_initial_state_vec(
    ...
    def retime_traj(self, traj, vel=0.01, inds=None, minpts=10):
        ...
        for step in traj:
            ...
            rot_interp = scipy.interpolate.interp1d(
                np.array(xpts), rotpts, kind="next", bounds_error=False, axis=0
            )
            ...
            for step in out:
                self.sawyer.openrave_body.set_pose(
                    step[self.state_inds["sawyer", "pose"]]
                )
                self.sawyer.openrave_body.set_dof(
                    {"right": step[self.state_inds["sawyer", "right"]]}
                )
                info = self.sawyer.openrave_body.fwd_kinematics("right")
                out[:, self.state_inds["sawyer", "right_ee_pos"]] = info["pos"]
                out[
                    :, self.state_inds["sawyer", "right_ee_rot"]
    ...
    def feasible_state(self, x, targets):
        ...
        for obj in opts[OBJ_ENUM]:
            ...
            if pos[2] < 0.6 or (pos[1] < 0 and pos[2] < 0.8):
                return False
        return True
