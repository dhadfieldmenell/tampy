...
class RolloutServer(Server):
    def __init__(self, hyperparams):
        ...
        self.prim_decay = hyperparams.get("prim_decay", 1.0)
        self.prim_first_wt = hyperparams.get("prim_first_wt", 1.0)
        ...
        self.hl_test_log = (
            LOG_DIR
            + hyperparams["weight_dir"]
            + "/"
            + str(self.id)
            + "_"
            + "hl_test_{0}{1}log.npy"
        )
        self.fail_log = (
            LOG_DIR
            + hyperparams["weight_dir"]
            ...
            + str(self.id)
            + "_"
            + "failure_{0}_log.txt".format(self.id)
        )
        self.fail_data_file = (
            LOG_DIR
            + hyperparams["weight_dir"]
            ...
            + str(self.id)
            + "_"
            + "failure_{0}_data.txt".format(self.id)
        )
        self.expert_data_file = (
            LOG_DIR + hyperparams["weight_dir"] + "/" + str(self.id) + "_exp_data.npy"
        )
        self.hl_data = []
        self.fail_data = []
        self.postcond_info = []
        ...
        self.dagger_log = (
            LOG_DIR
            + hyperparams["weight_dir"]
            + "/RolloutInfo_{0}_log.txt".format(self.id)
        )
        ...
        self.suc_trajs = (
            []
        )  # Each entry should be (traj, list of ts, list of prob under policy)
        self.failed_trajs = (
            []
        )  # Each entry should be (traj, list of ts, list of prob under policy)
    ...
    def init_supervisor(self):
        self.rollout_supervisor = RolloutSupervisor(
            self.agent,
            self.policy_opt,
            self.hyperparams,
            self.check_precond,
    ...
    def hl_log_prob(self, path):
        ...
        for sample in path:
            for t in range(sample.T):
                ...
                distrs = self.policy_opt.task_distr(sample.get_prim_obs(t=t), eta=1.0)
                ind = 0
                p = 1.0
                for d in distrs:
    ...
    def get_task(self, state, targets, prev_task, soft=False, eta=None):
        ...
        self.agent.fill_sample(
            0, sample, state.copy(), 0, prev_task, fill_obs=True, targets=targets
        )
        distrs = self.primitive_call(
            sample.get_prim_obs(t=0), soft, eta=eta, t=0, task=prev_task
        )
        ...
        for i, d in enumerate(distrs):
            ...
            if not np.isscalar(opts[enum]):
                ...
                if not len(inds):
                    raise Exception(
                        "Bad network output in get_task: {} {}".format(i, d)
                    )
    ...
    def plan_from_fail(self, augment=False, mode="start"):
        ...
        if val < 1:
            if mode == "start":
                ...
                wts = np.exp(np.arange(len(path)) / 5.0)
                wts /= np.sum(wts)
                s = np.random.choice(list(range(len(path))), p=wts)
                t = np.random.randint(path[s].T)
            elif mode == "task":
                breaks = find_task_breaks(path)
                cost_f = lambda x, task: self.agent.cost_f(
                    x, task, condition=0, targets=path[0].targets, active_ts=(-1, -1)
                )
                ...
                for s, sample in enumerate(path):
                    for t in range(sample.T):
                        if post_cost > 0:
                            break
                        if not self.agent.compare_tasks(
                            cur_task, sample.get(FACTOREDTASK_ENUM, t=t)
                        ):
                            post_cost = min(
                                [
                                    self.agent.postcond_cost(
                                        sample, task=tuple(cur_task.astype(int)), t=i
                                    )
                                    for i in range(
                                        max(0, t - delta), min(sample.T - 1, t + delta)
                                    )
                                ]
                            )
                            cur_task = sample.get(FACTOREDTASK_ENUM, t=t)
            ...
            prob = plan.prob
            domain = plan.domain
            abs_prob = self.agent.hl_solver.translate_problem(
                prob, goal=goal, initial=initial
            )
    ...
    def test_hl(
        ...
        for t in range(n, n_targs[-1]):
            ...
            targets[
                self.agent.target_inds["{0}_end_target".format(obj_name), "value"]
            ] = x0[self.agent.state_inds[obj_name, "pose"]]
        ...
        val, path = self.test_run(
            x0,
            targets,
            rlen,
            hl=True,
            soft=self.config["soft_eval"],
            eta=eta,
            lab=-5,
            hor=hor,
        )
        ...
        true_disp = self.agent.distance_to_goal(
            end_state, targets
        )  # np.min(np.min([[self.agent.goal_f(0, step.get(STATE_ENUM, t), targets, cont=True) for t in range(step.T)] for step in path]))
        true_val = np.max(
            np.max(
                [
                    [
                        1 - self.agent.goal_f(0, step.get(STATE_ENUM, t), targets)
        ...
        subgoal_suc = 1 - self.agent.goal_f(
            0, np.concatenate([s.get(STATE_ENUM) for s in path]), targets
        )
        anygoal_suc = 1 - self.agent.goal_f(
            0, np.concatenate([s.get(STATE_ENUM) for s in path]), targets, anywhere=True
        )
        subgoal_dist = self.agent.goal_f(
            0, np.concatenate([s.get(STATE_ENUM) for s in path]), targets, cont=True
        )
        ncols = (
            1.0
            if len(path) > 1
            and any(
                [len(np.where(sample.col_ts > 0.99)[0]) > 3 for sample in path[:-1]]
            )
            else 0.0
        )  # np.max([np.max(sample.col_ts) for sample in path])
        plan_suc_rate = (
            np.nan
            if self.agent.n_plans_run == 0
            else float(self.agent.n_plans_suc_run) / float(self.agent.n_plans_run)
        )
        ...
        res.append(s[0])
        if save:
            if all([s.opt_strength == 0 for s in path]):
                self.hl_data.append(res)
            if val > 1 - 1e-2:
                print("-----> SUCCESS! Rollout succeeded in test!", self.id)
            # if self.use_qfunc: self.log_td_error(path)
            np.save(
                self.hl_test_log.format("", "rerun_" if ckpt_ind is not None else ""),
                np.array(self.hl_data),
            )
        ...
        if self.render and save_video:
            ...
            if opt_path is not None:
                self.save_video(opt_path, val > 0, lab="_mp")
    ...
    def run(self):
        ...
        while not self.stopped:
            ...
            if self.run_hl_test or time.time() - self.last_hl_test > 120:
                self.agent.replace_cond(0)
                self.agent.reset(0)
                n_plans = self._hyperparams["policy_opt"]["buffer_sizes"][
                    "n_plans"
                ].value
    ...
    def test_run(
        ...
        while (
            t < max_t and val < 1 - 1e-2 and self.agent.feasible_state(state, targets)
        ):
    ...
    def get_log_info(self):
        ...
        info["dagger_success"] = (
            np.mean(self.postcond_info[-wind:]) if len(self.postcond_info) else 0.0
        )
        for key in self.fail_types:
        ...
        for key in self.task_successes:
            if len(self.task_successes[key]):
                info["{0}_successes".format(key)] = np.mean(
                    self.task_successes[key][-wind:]
                )
            else:
                info["{0}_successes".format(key)] = 0.0
