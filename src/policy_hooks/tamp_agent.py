...
class TAMPAgent(Agent, metaclass=ABCMeta):
    def __init__(self, hyperparams):
        ...
        for condition in range(len(self.x0)):
            ...
            for target_name in self.targets[condition]:
                target_vec[self.target_inds[target_name, "value"]] = self.targets[
                    condition
                ][target_name]
        ...
        self.discrete_opts = [
            enum for enum, enum_opts in opts.items() if hasattr(enum_opts, "__len__")
        ]
        self.continuous_opts = [
            enum
            for enum, enum_opts in opts.items()
            if not hasattr(enum_opts, "__len__")
        ]
        ...
        if "ll_solver_type" in self._hyperparams["master_config"]:
            self.ll_solver = self._hyperparams["master_config"]["ll_solver_type"](
                self._hyperparams
            )
        else:
    ...
    def clear_samples(self, keep_prob=0.0, keep_opt_prob=1.0):
        for task in self.task_list:
            ...
            self.optimal_samples[task] = random.sample(
                self.optimal_samples[task], n_opt_keep
            )
    ...
    def clear_task_paths(self, keep_prob=0.0):
    ...
    def animate_sample(self, sample):
                ...
                for param_name, attr in self.state_inds:
                    if attr == "pose":
                        self.mjc_env.set_item_pos(
                            param_name,
                            mp_state[self.state_inds[param_name, attr]],
                            mujoco_frame=False,
                        )
                    elif attr == "rotation":
                        self.mjc_env.set_item_rot(
                            param_name,
                            mp_state[self.state_inds[param_name, attr]],
                            mujoco_frame=False,
                            use_euler=True,
        ...
        for t in range(sample.T):
            for p_name, a_name in self.state_inds:
                if a_name == "rotation" or plan.params[p_name].is_symbol():
                    continue
                if a_name == "pose":
    ...
    def policies_initialized(self):
        for policy_name, policy in self.policies.items():
            if policy_name == "cont":
                continue
            if not self.policy_initialized(policy):
                return False
        return True
    ...
    def _sample_task(
        ...
        plan = (
            self.plans[onehot_task]
            if onehot_task in self.plans
            else self.plans[task[0]]
        )
        ...
        if hor is None:
            hor = (
                plan.horizon
                if task_f is None
                else max([p.horizon for p in list(self.plans.values())])
            )
        ...
        for t in range(0, self.T):
            ...
            self.fill_sample(
                condition, sample, cur_state.copy(), t, task, fill_obs=True
            )
            if task_f is not None:
                ...
                if policies is not None:
                    policy = policies[taskname]
                self.fill_sample(
                    condition, sample, cur_state.copy(), t, task, fill_obs=False
                )

            prev_vals = {}
            if (
                policies is not None
                and "cont" in policies
                and len(self.continuous_opts)
            ):
    ...
    def resample(self, sample_lists, policy, num_samples):
        ...
        for slist in sample_lists:
            if hasattr(slist, "__len__") and not len(slist):
                continue
            new_samples = []
            for i in range(num_samples):
                ...
                s = self.sample_task(
                    policy, s.condition, s.get_X(t=0), s.task, noisy=True
                )
    ...
    def sample_optimal_trajectory(
        self, state, task, condition, opt_traj=[], traj_mean=[], fixed_targets=[]
    ):
        raise NotImplementedError("This should be defined in child")

    @abstractmethod
    def solve_sample_opt_traj(
        self, state, task, condition, traj_mean=[], fixed_targets=[]
    ):
        raise NotImplementedError
    ...
    def randomize_init_state(self, condition=0):
        ...
        self.init_vecs[condition] = self.prob.get_random_initial_state_vec(
            self.config, self.num_objs, self.targets, self.dX, self.state_inds, 1
        )[0]
        ...
        if OBJ_ENUM in prim_choices and curric_step > 0:
            ...
            for j in inds:
                ...
                if "{0}_end_target".format(obj) not in self.targets[cond]:
                    continue
                if i >= len(prim_choices[OBJ_ENUM]) - curric_step:
                    break
    ...
    def perturb_init_states(self, perturb_var=0.02):
        for c in range(len(self.x0)):
            ...
            for obj in self.obj_list:
                ...
                if obj.is_symbol() or obj._type == "Robot":
                    continue
                x0[self.state_inds[obj, "pose"]] += np.random.normal(
                    0, perturb_var, obj_p.pose.shape[0]
                )
    ...
    def perturb_targets(self, perturb_var=0.02):
        for c in range(len(self.x0)):
            for target_name in self.targets[c]:
                ...
                self.targets[c][target_name] += np.random.normal(
                    0, perturb_var, target_p.value.shape[0]
                )
                self.target_vecs[c][
                    self.target_inds[target_name, "value"]
                ] = self.targets[c][target_name]
    ...
    def get_trajectories(self, sample=None, mp_state=None):
        ...
        for param_name, attr in self.state_inds:
            if plan.params[param_name].is_symbol():
                continue
    ...
    def relabel_path(self, path):
        ...
        if ONEHOT_GOAL_ENUM in self._hyperparams["sensor_dims"] and np.all(
            goal[ONEHOT_GOAL_ENUM] == 0.0
        ):
            return []
        ...
        for plan in plans:
            for pname, aname in self.state_inds:
                ...
                if state is not None:
                    getattr(plan.params[pname], aname)[:, st] = state[
                        self.state_inds[pname, aname]
                    ]
            ...
            for pname, aname in self.target_inds:
                getattr(plan.params[pname], aname)[:, 0] = targets[
                    self.target_inds[pname, aname]
                ]
    ...
    def encode_action(self, action, next_act=None):
        ...
        for enum in prim_choices:
            if enum is TASK_ENUM or not hasattr(prim_choices[enum], "__len__"):
                continue
            l.append(-1)
    ...
    def backtrack_solve(
        ...
        for a in range(anum, len(plan.actions)):
            ...
            if len(traj) > act_et:
                ...
                if last_t < 0:
                    ...
                    sample.use_ts[last_t:] = 0.0
                    sample.prim_use_ts[last_t:] = 0.0
                cost = (
                    self.postcond_cost(sample, task, sample.T - 1, x0=x0)
                    if last_t is None
                    else 0
                )
                # cost = self.postcond_cost(sample, task, sample.T-1, x0=x0)
                ref_traj, _, labels, _ = self.reverse_retime(
                    [sample], (act_st, act_et), label=True, T=last_t
                )
                ...
                for cur_step in range(2, act_et - act_st - 1):
                    ...
                    try:
                        ...
                        success = self.ll_solver._backtrack_solve(
                            plan, anum=a, amax=a, n_resamples=0, init_traj=[], st=cur_t
                        )
                    except AttributeError as e:
                        success = False
                    except Exception as e:
                        print(
                            "Exception in backup solve for",
                            cur_x0,
                            task,
                            plan.actions[a],
                            e,
                    ...
                    if success:
                        ...
                        next_path, next_x0 = self.run_action(
                            plan,
                            a,
                            cur_x0,
                            perm_targets,
                        ...
                        if verbose:
                            print(
                                self.process_id,
                                "Adding backup solve traj for {}...".format(task),
                            )
                        ...
                        if verbose:
                            print(
                                self.process_id,
                                "Failed backup solve traj for {}...".format(task),
                            )
            ...
            if rollout and cost == 0:
                ...
                sample.success = 1.0 - self.goal_f(0, next_x0, targets)
                set_params_attrs(
                    plan.params, self.state_inds, ref_x0, act_et, plan=plan
                )
            else:
                set_params_attrs(
                    plan.params, self.state_inds, prev_x0, act_st, plan=plan
                )
                ...
                try:
                    ...
                    print(
                        ("Opt Exception in full solve for", x0, task, plan.actions[a]),
                        st,
                    )
                    success = False
                except Exception as e:
                    ...
                    print(
                        ("Exception in full solve for", x0, task, plan.actions[a]),
                        e,
                        st,
                    )
                    success = False
        ...
        rollout_success = (
            len(path) and self.goal_f(0, path[-1].end_state, targets) < 1e-3
        )
        if success:
            self.n_plans_run += 1
        ...
        if len(smooth_cnts):
            print(
                self.process_id, "N smooth:", smooth_cnts, time.time() - init_t, verbose
            )
        print(
            (
                "Plans run vs. success:",
                self.n_plans_run,
    ...
    def run_plan(
        ...
        if permute:
            ...
            for pname, aname in self.state_inds:
                if pname in perm:
                    x0[self.state_inds[perm[pname], aname]] = getattr(
                        plan.params[pname], aname
                    )[:, start_ts]
        if reset:
            self.reset_to_state(x0)
        ...
        for ind, s in enumerate(path):
            ...
            cost = self.postcond_cost(
                end_s, end_s.task, end_s.T - 1, debug=False, x0=x0s[ind]
            )
            if cost < 1e-3:
                s._postsuc = True
        ...
        if label.find("dagger") >= 0 and self.dagger_window > 0:
            ...
            path = path[:1]
            path[0].use_ts[self.dagger_window :] = 0.0
            path[0].prim_use_ts[self.dagger_window :] = 0.0
        ...
        if len(path):
            for ind, s in enumerate(path):
                ...
                if cost < 1e-3:
                    ...
                    log_info[
                        "{}_opt_rollout_success".format(self.task_list[s.task[0]])
                    ].append(1)
                elif path[-1].success > 0.99:
                    print(
                        "Adding path w/postcond failure?",
                        self.task_list[s.task[0]],
                        self.process_id,
                    )
                    log_info[
                        "{}_opt_rollout_success".format(self.task_list[s.task[0]])
    ...
    def run_action(
        ...
        for pname, attr in self.state_inds:
            if plan.params[pname].is_symbol():
                continue
            opt_traj[:, self.state_inds[perm.get(pname, pname), attr]] = getattr(
                plan.params[pname], attr
            )[:, st : et + 1].T
            x0[self.state_inds[perm.get(pname, pname), attr]] = static_x0[
                self.state_inds[pname, attr]
            ]
            base_x0[self.state_inds[perm.get(pname, pname), attr]] = static_base[
                self.state_inds[pname, attr]
            ]
            cur_hist[:, self.state_inds[perm.get(pname, pname), attr]] = static_hist[
                :, self.state_inds[pname, attr]
            ]
        ...
        if self.retime:
            ...
            if np.any(np.isnan(opt_traj)):
                print("NAN in opt traj")
            ...
            while t < len(new_traj) - 1:
                traj = new_traj[t : t + T]
                sample = self.sample_optimal_trajectory(
                    x0, task, 0, traj, targets=targets
                )
                path.append(sample)
                sample.discount = 1.0
                sample.opt_strength = 1.0
                ...
                sample.success = 1.0 - self.goal_f(0, x0, sample.targets)
        else:
            sample = self.sample_optimal_trajectory(
                x0, task, 0, opt_traj, targets=targets
            )
            path.append(sample)
            sample.discount = 1.0
            sample.opt_strength = 1.0
            ...
            sample.success = 1.0 - self.goal_f(0, x0, targets)
        ...
        if nzero > 0 and add_noop:
            ...
            zero_sample = self.sample_optimal_trajectory(
                path[-1].end_state, task, 0, zero_traj, targets=targets
            )
            x0 = zero_sample.end_state  # sample.get_X(sample.T-1)
            zero_sample.use_ts[:] = 0.0
            zero_sample.use_ts[:nzero] = 1.0
            ...
            zero_sample.success = 1.0 - self.goal_f(0, x0, targets)
        ...
        end_s.task_end = True
        cost = self.postcond_cost(
            end_s, task, end_s.T - 1, debug=False, x0=base_x0, tol=1e-3
        )
        ...
        for ind, s in enumerate(path):
            ...
            if cost < 1e-4:
                ...
                s._postsuc = False
                if save:
                    print("Ran opt path w/postcond failure?", task, self.process_id)
        ...
        for pname, attr in self.state_inds:
            if plan.params[pname].is_symbol():
                continue
            x0[self.state_inds[rev_perm.get(pname, pname), attr]] = static_x0[
                self.state_inds[pname, attr]
            ]
            self._x_delta[
                :, self.state_inds[rev_perm.get(pname, pname), attr]
            ] = static_hist[:, self.state_inds[pname, attr]]
    ...
    def permute_ts(self, sample, ts):
        ...
        for pname, attr in self.state_inds:
            if plan.params[pname].is_symbol():
                continue
            perm_x[self.state_inds[perm.get(pname, pname), attr]] = cur_x[
                self.state_inds[perm.get(pname, pname), attr]
            ]
            if hist is not None:
                self._x_delta[:, self.state_inds[perm.get(pname, pname), attr]] = hist[
                    :, self.state_inds[perm.get(pname, pname), attr]
                ]

        self.fill_sample(
            0, sample, perm_x, ts, perm_tasks[0], targets=perm_targets, fill_obs=True
        )
        self._x_delta[:] = prev_hist
    ...
    def project_onto_constr(self, x0, plan, targets, ts=(0, 0)):
        x0 = x0.copy()
        for pname, aname in self.state_inds:
            plan.params["robot_init_pose"].value[:, 0] = x0[
                self.state_inds["pr2", "pose"]
            ]
            if plan.params[pname].is_symbol():
                continue
            getattr(plan.params[pname], aname)[:, 0] = x0[self.state_inds[pname, aname]]
            plan.params[pname]._free_attrs[aname][:, 0] = 1.0
            if "{0}_init_target".format(pname) in plan.params:
                plan.params["{0}_init_target".format(pname)].value[:, 0] = x0[
                    self.state_inds[pname, aname]
                ]
        suc = self.solver.solve(
            plan, traj_mean=np.array(x0).reshape((1, -1)), active_ts=(0, 0)
        )
        for pname, aname in self.state_inds:
            if plan.params[pname].is_symbol():
                continue
    ...
    def resample_hl_plan(self, plan, targets, n=5):
        x0s, _ = self.prob.get_random_initial_state_vec(
            self.config, None, self.symbolic_bound, self.state_inds, n
        )
        nsuc = 0
        for x0 in x0s:
            for pname, aname in self.state_inds:
                ...
                if plan.params[pname].is_symbol():
                    continue
                getattr(plan.params[pname], aname)[:, 0] = x0[
                    self.state_inds[pname, aname]
                ]
                plan.params[pname]._free_attrs[aname][:, 0] = 1.0
                if "{0}_init_target".format(pname) in plan.params:
            ...
            for pname, aname in self.state_inds:
                if plan.params[pname].is_symbol():
                    continue
                x0[self.state_inds[pname, aname]] = getattr(plan.params[pname], aname)[
                    :, 0
                ]
    ...
    def _failed_preds(
        ...
        failed_preds = [
            p for p in failed_preds if (p[1]._rollout or not type(p[1].expr) is EqExpr)
        ]
        if debug:
    ...
    def get_annotated_image(self, s, t, cam_id=None):
        ...
        textover2 = self.mjc_env.get_text_overlay(
            body="{0}; {1}".format(u, pos), position="bottom left"
        )
    ...
    def reverse_retime(self, samples, ts, label=False, start_t=0, T=None):
        ...
        if label:
            return np.array(traj), labels, rev_labels, env_state
    ...
    def save_video(
        self, rollout, savedir, success=None, ts=None, lab="", annotate=True, st=0
    ):
        init_t = time.time()
        ...
        fname = savedir + "/{0}_{1}_{2}_{3}_{4}.npy".format(
            self.process_id,
            self.cur_vid_id,
            suc_flag,
            lab,
        ...
        save_video(
            fname,
            dname=self.config["master_config"]["descr"],
            arr=np.array(buf),
            savepath=savedir,
        )
