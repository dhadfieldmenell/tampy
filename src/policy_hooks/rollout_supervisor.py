...
from sco_gurobi.expr import EqExpr
...
class RolloutSupervisor:
    ...
    def task_f(self, sample, t, curtask):
        ...
        if self.check_postcond:
            postcost = self.agent.postcond_cost(
                sample, task, t, x0=self.switch_x[-1], tol=self.tol
            )
            if postcost < 1e-4:
                if self.neg_postcond:
                    self.neg_samples.append((sample, t, truetask))

            if task != curtask:
                postcost = self.agent.postcond_cost(
                    sample, curtask, t, x0=self.switch_x[-1], tol=self.tol
                )
                if postcost > 0:
                    ...
                    self.postcond_costs[self.agent.task_list[curtask[0]]].append(
                        postcost
                    )
        ...
        if task != curtask and self.check_precond:
            ...
            while n_tries < 20 and task != curtask and precost > 0:
                if self.neg_precond:
                    self.neg_samples.append((sample, t, truetask))
                task = self.get_task(
                    sample.get_X(t=t), sample.targets, curtask, True, eta=cur_eta
                )
                truetask = task
                task = tuple([val for val in task if np.isscalar(val)])
                precost = self.agent.precond_cost(sample, task, t, tol=self.tol)
    ...
    def rollout(self, x, targets, node):
        ...
        self.postcond_costs[self.agent.task_list[self.cur_tasks[-1][0]]].append(
            postcost
        )
        ...
        if val >= 0.999:
            print(
                "Success in rollout. Pre: {} Post: {} Mid: {} Random: {}".format(
                    self.check_precond,
                    self.check_postcond,
                    self.check_midcond,
                    self.check_random,
    ...
    def primitive_call(
        self, prim_obs, soft=False, eta=1.0, t=-1, task=None, adj_eta=False
    ):
        if adj_eta:
            eta *= self.agent.eta_scale
    ...
    def parse_midcond(self, path):
        ...
        traj, steps, _, env_states = self.agent.reverse_retime(
            path[bad_pt[0] :], (0, plan.horizon - 1), label=True, start_t=st
        )
        ...
        set_params_attrs(
            plan.params, self.agent.state_inds, path[bad_pt[0]].get_X(t=st), 0
        )
        self.agent.set_symbols(plan, self.cur_tasks[-1], targets=targets)
        failed_preds = plan.get_failed_preds(
            tol=self.tol, active_ts=(0, plan.horizon - 1)
        )
        failed_preds = [
            p
            ...
            if (
                p[1]._rollout
                or (not p[1]._nonrollout and type(p[1].expr) is not EqExpr)
            )
        ]
        ...
        for s, t, fail_type in train_pts:
            ...
            if val < 1:
                ...
                abs_prob = self.agent.hl_solver.translate_problem(
                    concr_prob, initial=initial, goal=goal
                )
                set_params_attrs(
                    concr_prob.init_state.params, self.agent.state_inds, state.copy(), 0
                )
                hlnode = HLSearchNode(
                    abs_prob,
                    node.domain,
    ...
    def save_video(self, rollout, success=None, ts=None, lab="", annotate=True, st=0):
        if not self.hyperparams["load_render"]:
            return
        ...
        fname = self.video_dir + "/{0}_{1}_{2}_{3}{4}_{5}.npy".format(
            self.agent.process_id,
            self.cur_vid_id,
            suc_flag,
            lab,
            ...
            for t in ts_range:
                ...
                for ind, cam_id in enumerate(cam_ids):
                    if annotate and ind == 0:
                        ims.append(
                            self.agent.get_annotated_image(step, t, cam_id=cam_id)
                        )
                    else:
        ...
        save_video(
            fname,
            dname=self._hyperparams["descr"],
            arr=np.array(buf),
            savepath=self.video_dir,
        )
