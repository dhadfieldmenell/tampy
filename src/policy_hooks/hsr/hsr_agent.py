...
class HSRAgent(TAMPAgent):
    ...
    def fill_sample(self, cond, sample, mp_state, t, task, fill_obs=False, targets=None):
        ...
        if self.discrete_prim:
            ...
            obj_pose = mp_state[self.state_inds[obj_name, 'pose']] - mp_state[self.state_inds['hsr', 'pose']]
            targ_pose = targets[self.target_inds[targ_name, 'value']] - mp_state[self.state_inds['hsr', 'pose']]
        ...
        if INGRASP_ENUM in self._hyperparams['sensor_dims']:
            ...
            for i, o in enumerate(prim_choices[OBJ_ENUM]):
                if np.all(np.abs(mp_state[self.state_inds[o, 'pose']] - mp_state[self.state_inds['hsr', 'pose']] - grasp) < NEAR_TOL):
                    vec[i] = 1.
    ...
    def _failed_preds(self, Xs, task, condition, active_ts=None, debug=False, targets=[]):
        ...
        plan.params['hsr'].gripper[:,active_ts[0]] = Xs[0][self.state_inds['hsr', 'gripper']]
        for param_name in plan.params:
            param = plan.params[param_name]
            if param.is_symbol(): continue
            if param._type == 'Can' and '{0}_init_target'.format(param_name) in plan.params:
    ...
    def reset_to_state(self, x):
        ...
        for param_name, attr in self.state_inds:
            if attr == 'pose':
                ...
                self.mjc_env.set_item_pos(param_name, np.r_[pos, 0.], mujoco_frame=False, forward=False)
