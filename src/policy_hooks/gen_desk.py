    ...
    for k in data:
        ...
        if 'goal' in columns:
            ...
            if rolling:
                ...
                print('Rolling mean on', numeric_cols)
                #dfs = [pd.DataFrame(y) for x, y in pd_frame.groupby(['exp id', 'description'], as_index=False)]
                dfs = [pd.DataFrame(y) for x, y in pd_frame.groupby(['exp id', 'Problem'], as_index=False)]
                for df in dfs:
                    wind = int(window)
...
def gen_data_plots(xvar, yvar, keywords=[], lab='rollout', inter=1., 
    ...
    if split_goal:
        ...
        plot(data, ['Problem', 'key', 'exp id']+xvars+flat_yvar_labs, '{0}_{1}_vs_{1}'.format(prefix, xvar, ylabel), xvars, yvar_labs, separate=separate, keyind=keyind, inter=inter, rolling=rolling, window=window, xlim=xlim, ylim=ylim, fname=fname)
