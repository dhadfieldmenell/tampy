...
CLOTH_INIT_X_RANGE = [0.35, 0.75]
CLOTH_INIT_Y_RANGE = [0.40, 1.0]
...
def generate_cond(num_cloths):
    ...
    plan.params['robot_init_pose'].lArmPose[:,0] = [0, -0.5, 0, 0, 0, 0, 0]
    plan.params['robot_init_pose'].lGripper[:,0] = 0
    plan.params['robot_init_pose'].rArmPose[:,0] = [0, -0.5, 0, 0, 0, 0, 0]
    plan.params['robot_init_pose'].rGripper[:,0] = 0

    plan.params['robot_end_pose'].lArmPose[:,0] = [0.785, 0, 0, 0, 0, 0, 0]
    plan.params['robot_end_pose'].lGripper[:,0] = 0
    plan.params['robot_end_pose'].rArmPose[:,0] = [-0.785, 0, 0, 0, 0, 0, 0]
...
def get_randomized_initial_state_multi_step(plan, plan_num):
    ...
    for c in range(num_cloths-1, -1, -1):
        ...
        X[:, plan.state_inds[('cloth_putdown_end_{0}'.format(c), 'lGripper')]] = 0
        X[:, plan.state_inds[('cloth_putdown_end_{0}'.format(c), 'rArmPose')]] = R_ARM_PUTDOWN_END
        X[:, plan.state_inds[('cloth_putdown_end_{0}'.format(c), 'rGripper')]] = 0
    ...
    for i in range(1, num_cloths):
        ...
        X[i, plan.state_inds[('baxter', 'lGripper')]] = 0.2
        X[i, plan.state_inds[('baxter', 'rArmPose')]] = R_ARM_PUTDOWN_END
        X[i, plan.state_inds[('baxter', 'rGripper')]] = 0.2
