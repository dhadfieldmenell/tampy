                    ...
                    if len(next_data):
                        ...
                        for pt in r_data:
                            ...
                            if full_exp.find('full_feed') >= 0:
                                pt['opt duration per ts'] = 1.35 / pt['opt duration per ts']
                                pt['description'] = 'Robosuite'
                            elif full_exp.find('panda') >= 0:
                                pt['opt duration per ts'] = 2.25 / pt['opt duration per ts']
    ...
    for k in data:
        ...
        if 'goal' in columns:
            ...
            if rolling:
                ...
                print('Rolling mean on', numeric_cols)
                #dfs = [pd.DataFrame(y) for x, y in pd_frame.groupby(['exp id', 'description'], as_index=False)]
                dfs = [pd.DataFrame(y) for x, y in pd_frame.groupby(['exp id', 'Domain'], as_index=False)]
                for df in dfs:
                    wind = int(window)
            ...
            for xind, xv in enumerate(xvars):
                for yind, yv in enumerate(yvars):
                    ...
                    if ylim is not None:
                        ...
                        sns_plot.fig.axes[-1].set_yticks(np.arange(0, ylim[yind][1], ylim[yind][1]/10.))
            naxs = len(sns_plot.fig.get_axes())
            sns_plot.fig.get_axes()[0].legend(bbox_to_anchor=[-0.0, -0.], loc='lower left', prop={'size': 12}, facecolor='white')
...
def gen_label(exp_dir, label_vars=[], split_runs=False, run_ind=0):
    ...
    for v in label_vars:
        ...
        if args[v].find('robodesk') >= 0:
            label = 'Robodesk'
            break
        elif args[v].find('full_feed') >= 0:
            label = 'Robosuite'
            break
...
def gen_data_plots(xvar, yvar, keywords=[], lab='rollout', inter=1., 
    ...
    if split_goal:
        ...
        plot(data, ['Domain', 'key', 'exp id']+xvars+flat_yvar_labs, '{0}_{1}_vs_{1}'.format(prefix, xvar, ylabel), xvars, yvar_labs, separate=separate, keyind=keyind, inter=inter, rolling=rolling, window=window, xlim=xlim, ylim=ylim, fname=fname)
