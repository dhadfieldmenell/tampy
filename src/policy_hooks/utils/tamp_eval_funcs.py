...
def ts_constr_violations(plan, ts, include=[], exclude=[]):
...
def check_constr_violation(plan, active_ts=None, exclude=[]):
    if active_ts is None:
        active_ts = (0, plan.horizon)

    viol = np.zeros((active_ts[1]-active_ts[0]))
    for t in range(active_ts[0], active_ts[1]):
        viol[t-active_ts[0]] += sum(ts_constr_violations(plan, t, exclude).values())
