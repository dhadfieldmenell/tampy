    ...
    def __init__(self, loss_scalar, solver_name='adam', base_lr=None, lr_policy=None,
        ...
        if weight_decay is not None:
            ...
            for var in trainable_vars:
                for ind in range(len(self.loss_scalar)):
                    self.loss_scalar[ind] += self.weight_decay*tf.nn.l2_loss(var)

        solver_ops = [self.get_solver_op(loss=loss) for loss in self.loss_scalar]
        self.solver_op = tf.group(solver_ops)
        if fc_vars is not None:
            self.fc_vars = fc_vars
            self.last_conv_vars = last_conv_vars
            self.fc_solver_op = tf.group([self.get_solver_op(var_list=fc_vars, loss=loss) for loss in self.loss_scalar])
