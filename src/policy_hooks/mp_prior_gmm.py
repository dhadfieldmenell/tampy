...
class PolicyMPPriorGMM(object):
    ...
    def update(self, samples, policy_opt, mode='add', task=''):
        """
        ...
        """
        X, obs = samples.get(STATE_ENUM), samples.get(STATE_ENUM)
        ...
        N, T = self.X.shape[:2]
        dO = 2 * self.X.shape[2]
        XU = np.reshape(np.concatenate([self.X, self.obs], axis=2), [T * N, dO])
