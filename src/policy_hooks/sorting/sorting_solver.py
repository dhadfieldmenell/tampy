...
from policy_hooks.policy_predicates import (
    BaxterPolicyPredicate,
    BaxterPolicyEEPredicate,
)
...
class BaxterPolicySolver(RobotLLSolver):
    ...
    def train_policy(self, num_cloths, hyperparams=None):
        ...
        (
            ...
            self.action_inds,
            self.symbolic_bound,
        ) = utils.get_state_action_inds(
            plans[0], state_vector_include, action_vector_include
        )
        ...
        for plan in plans:
            ...
            utils.fill_vector(
                [p for p in list(plan.params.values()) if not p.is_symbol()],
                self.state_inds,
                x0[-1],
                0,
            )
        ...
        if not self.gps:
            ...
            for plan in plans:
                self._add_policy_constraints_to_plan(
                    plan, list(state_vector_include.keys())
                )
        # else:
    ...
    def optimize_against_global(self, plan, a_start=0, a_end=-1, cond=0):
        ...
        all_active_ts = (
            plan.actions[self.gps.agent.init_plan_states[cond][1][0]].active_timesteps[
                0
            ],
            plan.actions[self.gps.agent.init_plan_states[cond][1][1]].active_timesteps[
                1
            ],
        )
        ...
        while a_num < a_end:
            print(
                "Constraining actions {0} and {1} against the global policy.\n".format(
                    a_num, a_num + 1
                )
            )
    ...
    def _traj_policy_opt(self, plan, traj_mean, start_t, end_t):
        transfer_objs = []
        base_t = plan.actions[
            self.gps.agent.init_plan_states[self.gps.agent.current_cond][1][0]
        ].active_timesteps[0]
        for param_name, attr_name in list(self.action_inds.keys()):
            ...
            T = end_t - start_t + 1
            attr_val = traj_mean[
                self.action_inds[(param_name, attr_name)],
                start_t - base_t : end_t - base_t + 1,
            ].T
            K = attr_type.dim
            ...
            policy_transfer_coeff = self.policy_transfer_coeff / float(
                traj_mean.shape[1]
            )

            # QuadExpr is 0.5*x^Tx + Ax + b
            quad_expr = QuadExpr(
                2 * policy_transfer_coeff * Q,
                policy_transfer_coeff * A,
                policy_transfer_coeff * b,
            )
    ...
    def _add_policy_constraints_to_plan(self, plan, param_names):
        ...
        for action in plan.actions:
            ...
            policy_func = (
                lambda x: self.gps.alg_map[task]
                .policy_opt.task_map["policy"]
                .act(x, x, 0, 0)
            )  # The global policy should be time independent
            # pred = BaxterPolicyEEPredicate('PolicyPred', params, state_inds, action_inds, policy_func, dX, dU, self.config['policy_coeff'])
            pred = BaxterPolicyPredicate(
                "PolicyPred",
                params,
                state_inds,
                action_inds,
                ...
                dX,
                dU,
                self.config["policy_coeff"],
            )
            pred_dict = {
                "hl_info": "policy",
                "pred": pred,
                "negated": False,
                "active_timesteps": action.active_timesteps,
            }
    ...
    def _solve_policy_opt_prob(self, plan, global_traj_mean, active_ts, resample):
        ...
        if resample:
            ...
            if len(global_traj_mean):
                obj_bexprs.extend(
                    self._traj_policy_opt(
                        plan, global_traj_mean, active_ts[0], active_ts[1]
                    )
                )
            ...
            self._add_all_timesteps_of_actions(
                plan, priority=3, add_nonlin=False, active_ts=active_ts
            )
    ...
    def _add_policy_preds(self, plan, effective_timesteps):
        for action in plan.actions:
            if action.active_timesteps[1] <= effective_timesteps[0]:
                continue
            if action.active_timesteps[0] >= effective_timesteps[1]:
                continue
    ...
    def _add_policy_pred_dict(self, pred_dict, effective_timesteps):
        ...
        if expr is not None:
            for t in active_range:
                if t < effective_timesteps[0] or t > effective_timesteps[1]:
                    continue
    ...
    def _spawn_sco_var_for_policy_pred(self, pred, t):
        ...
        for param in pred.params:
            for attr in const.ATTR_MAP[param._type]:
                if (param.name, attr[0]) in pred.state_inds and (
                    param.name != "baxter" or attr[0] == "pose"
                ):
                    ll_p = self._param_to_ll[param]
                    if (param.name, attr[0]) in pred.action_inds:
                        x[pred.state_inds[(param.name, attr[0])]] = getattr(
                            ll_p, attr[0]
                        )[attr[1], t - self.ll_start]
                        v[pred.state_inds[(param.name, attr[0])]] = getattr(
                            param, attr[0]
                        )[attr[1], t]
                    else:
