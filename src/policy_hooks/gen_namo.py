            ...
            for dir_name in exp_dirs:
                ...
                for r in file_names:
                    ...
                    if len(next_data):
                        ...
                        for pt in r_data:
                            pt['exp id'] = 0
                            if full_exp.find('1obj') >= 0:
                                pt['description'] = '1 object (RGB)'
                            if full_exp.find('2obj') >= 0:
                                pt['description'] = '2 object (RGB)'
                            if full_exp.find('3obj') >= 0:
                                pt['description'] = '3 object (RGB)'
...
def plot(data, columns, descr, xvars, yvars, separate=True, keyind=0, inter=100, rolling=True, window=100, xlim=None, ylim=None, fname='', witherr=False, style=None):
    ...
    for k in data:
        ...
        if 'goal' in columns:
            ...
            for goal_ind, goal in enumerate(goals):
                ...
                for xv in xvars:
                    for yind, yv in enumerate(yvars):
                        ...
                        sns_plot.fig.axes[-1].set_title('2D', size=14)
                ...
                sns_plot.fig.get_axes()[0].title('2D Pickplace')
            ...
            sns_plot.fig.get_axes()[0].set_title('')
            naxs = len(sns_plot.fig.get_axes())
            sns_plot.fig.get_axes()[0].legend(bbox_to_anchor=[1., -0.], loc='lower right', prop={'size': 16, 'family': 'serif'}, facecolor='white')
            handles, labels = sns_plot.fig.get_axes()[0].get_legend_handles_labels()       
            sns_plot.fig.get_axes()[0].legend(handles=handles[1:], labels=labels[1:], facecolor="white", framealpha=0.85, prop={'size': 16, 'family': 'serif'})
...
def gen_label(exp_dir, label_vars=[], split_runs=False, run_ind=0):
    ...
    for v in label_vars:
        ...
        if args[v].find('human') >= 0:
            label = '3 object (6 Humans)'
            break
        if args[v].find('5obj') >= 0:
            label = '5 object'
            break
        if args[v].find('2obj') >= 0:
            label = '2 object (RGB)'
            break
        if args[v].find('1obj') >= 0:
            label = '1 object (RGB)'
            break
        if args[v].find('3obj') >= 0:
            label = '3 object (RGB)'
            break
...
if __name__ == '__main__':
    ...
    while not terminate:
        if not perpetual:
            terminate = True
        gen_data_plots(xvar='time', yvar=['success at end'], keywords=keywords, lab='test', label_vars=['descr'], separate=True, keyind=5, ylabel='succdataloadtargs', exclude=exclude, split_runs=False, include=include, inter=450, window=900, ylim=[(0.,1.), (0.,1.), (0, 1.), (0, 2.)], fname='endsucc_{}'.format(keywords[0]))
