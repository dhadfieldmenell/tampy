...
def build_conv_layers(input_layer, filter_sizes, num_filters):
    ...
    for i in range(n_conv):
        ...
        if i == 0 and cur_in > 3:
            layers = []
            for j in range(0, cur_in, 3):
                channels = min(3, cur_in-j)
                weights['conv_wc{0}_{1}'.format(i, j)] = init_weights([filter_size, filter_size, channels, num_filters[i]], name='conv_wc{0}_{1}'.format(i, j)) # 5x5 conv, 1 input, 32 outputs
                biases['conv_bc{0}_{1}'.format(i, j)] = init_bias([num_filters[i]], name='conv_bc{0}_{1}'.format(i, j))
                layers.append(conv2d(img=cur_in_layer[:,:,:,j:j+3], w=weights['conv_wc{0}_{1}'.format(i, j)], b=biases['conv_bc{0}_{1}'.format(i, j)], nonlin=nonlin, strides=strides))
            conv_layers.append(tf.concat(values=layers, axis=-1, name='in_concat'))
            cur_in_layer = conv_layers[-1]
            cur_in = 2*num_filters[i]
        else:
