    ...
    def __init__(self, config):
        ...
        for task in self.task_list:
            for c in range(num_cans):
                plans[task, '{0}{1}'.format(obj_type, c)] = prob.get_plan_for_task(task, ['{0}{1}'.format(obj_type, c), '{0}{1}_end_target'.format(obj_type, c)], num_cans, env, openrave_bodies)
                if env is None:
        ...
        state_vector_include, action_vector_include, target_vector_include = self.config['get_vector'](num_cans)

        self.dX, self.state_inds, self.dU, self.action_inds, self.symbolic_bound = utils.get_state_action_inds(plans.values()[0], self.config['robot_name'], ATTRMAP, state_vector_include, action_vector_include)

        self.target_dim, self.target_inds = utils.get_target_inds(plans.values()[0], self.config['attrmap'], target_vector_include)

        for i in range(conditions):
            targets.append(prob.get_end_targets(num_cans))
        
        x0 = prob.get_random_initial_state_vec(num_cans, targets, self.dX, self.state_inds, conditions)
        obj_list = ['{0}{1}'.format(obj_type, c) for c in range(num_cans)]
