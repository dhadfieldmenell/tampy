            ...
            for param, attr in self.action_inds:
                if attr == "gripper":
                    u[self.action_inds[param, attr]] = self.opt_traj[
                        t + 1, self.state_inds[param, attr]
                    ]
...
class NAMOSortingAgent(TAMPAgent):
    def __init__(self, hyperparams):
        ...
        for name in prim_options[OBJ_ENUM]:
            ...
            if name != "pr2":
                items.append(
                    {
                        "name": "{0}_end_target".format(name),
                        "type": "box",
                        "is_fixed": True,
                        "pos": (0, 0, 1.5),
                        "dimensions": (LOCAL_NEAR_TOL, LOCAL_NEAR_TOL, 0.05),
                        "rgba": tuple(cur_color),
                        "mass": 1.0,
                    }
                )
    ...
    def replace_targets(self, condition=0):
        ...
        if hasattr(self.prob, "NUM_TARGS"):
            for i in range(self.prob.NUM_TARGS, self.prob.NUM_OBJS):
                self.targets[condition]["can{0}_end_target".format(i)] = self.x0[
                    condition
                ][self.state_inds["can{0}".format(i), "pose"]]
    ...
    def dist_obs(
        ...
        if n_dirs <= 0:
            n_dirs = self.n_dirs
        if center not in plan.params:
            return LIDAR_DIST * np.ones(n_dirs)
            ...
            for p_name in params:
                ...
                if p.is_symbol():
                    ...
                    if (p_name, "pose") in self.state_inds:
                        p.openrave_body.set_pose(
                            np.r_[plan.params[p_name].pose[:, t], 5]
                        )
                    else:
                        p.openrave_body.set_pose(
                            np.r_[plan.params[p_name].pose[:, 0], 5]
                        )
    ...
    def trajopt_dist_obs(self, plan, t):
        ...
        for p_name in plan.params:
            ...
            if p.is_symbol() or p is pr2:
                continue
            if (p_name, "pose") in self.state_inds:
                p.openrave_body.set_pose(p.pose[:, t])
            else:
                p.openrave_body.set_pose(p.pose[:, 0])
            collisions = self._cc.BodyVsBody(
                pr2.openrave_body.env_body, p.openrave_body.env_body
            )
    ...
    def run_policy_step(self, u, x, plan=None, t=0, obj=None, grasp=None):
        ...
        if t < plan.horizon - 1:
            for param, attr in u_inds:
                if attr == "pose":
                    getattr(plan.params[param], attr)[:, t] = (
                        x[x_inds[param, attr]] + u[u_inds[param, attr]]
                    )
                elif attr == "gripper":
            ...
            check_col = (
                self.check_col
                and np.abs(new_pose[0]) > 5
                and (new_pose[1] > 1.5 or new_pose[1] < -4)
            )

            for param in list(plan.params.values()):
                if param._type == "Can":
                    disp = (
                        old_state[x_inds["pr2", "pose"]]
                        - old_state[x_inds[param.name, "pose"]]
                    )  # plan.params['pr2'].pose[:, t] - param.pose[:, t]
                    new_disp = plan.params["pr2"].pose[:, t] - param.pose[:, t]
                    mid_disp = (
                        plan.params["pr2"].pose[:, t]
                        - (param.pose[:, t] + old_state[x_inds[param.name, "pose"]])
                        / 2.0
                    )
                    # dist = np.linalg.norm(disp)
                    if grasp[1] < 0:
                        grasp_check = disp[1] < 0
                    if grasp[1] > 0:
                        grasp_check = disp[1] > 0
                    if grasp[0] < 0:
                        grasp_check = disp[0] < 0
                    if grasp[0] > 0:
                        grasp_check = disp[0] > 0
                    ...
                    if plan.params["pr2"].gripper[0, t] > GRIP_TOL and np.all(
                        np.abs(disp - grasp) < NEAR_TOL
                    ):
                    ...
                    if self.check_col:
                        ...
                        if param.name != self._in_gripper and (
                            np.linalg.norm(new_disp) < radius1 + radius2 - dtol
                            ...
                            col = 1.0 if obj is None or param.name != obj else 0.0
                            dx, dy = -1e1 * pr2_disp
                            zx, zy = param.pose[:, t]
                            x1, y1 = (
                                plan.params["pr2"].pose[:, t]
                            ...
                            if dx >= 0 and dy >= 0:
                                x = (
                                    D * dy
                                    + sy * dx * np.sqrt(r ** 2 * dr ** 2 - D ** 2)
                                ) / (dr ** 2)
                                y = (
                                    -D * dx
                                    + np.abs(dy) * np.sqrt(r ** 2 * dr ** 2 - D ** 2)
                                ) / (dr ** 2)
                            elif dx <= 0 and dy <= 0:
                                x = (
                                    D * dy
                            ...
                            if self._in_gripper is not None:
                                plan.params[self._in_gripper].pose[:, t] = plan.params[
                                    "pr2"
                                ].pose[:, t] - (
                                    old_state[x_inds["pr2", "pose"]]
                                    - old_state[x_inds[self._in_gripper, "pose"]]
                                )
                        ...
                        if (
                            self.in_gripper is not None
                            and self._in_gripper is not param.name
                            and np.linalg.norm(
                                plan.params[self._in_gripper].pose[:, t]
                                - param.pose[:, t]
                            )
                            < radius2 + radius2 - dtol
                        ):
                            col = 1.0
                            dx, dy = -1e1 * pr2_disp
                            zx, zy = param.pose[:, t]
                            x1, y1 = (
                                self.in_gripper.pose[:, t]
                                - [0.5 * dx, 0.5 * dy]
                                - [zx, zy]
                            )
                            # zx, zy = plan.params['pr2'].pose[:,t]
                            ...
                            plan.params["pr2"].pose[:, t] = self.in_gripper.pose[
                                :, t
                            ] - (
                                old_state[x_inds[self.in_gripper.name, "pose"]]
                                - old_state[x_inds["pr2", "pose"]]
                            )
                        else:
                            pass
                    ...
                    if plan.params["pr2"].gripper[0, t] < GRIP_TOL or (
                        obj is not None and param.name != obj
                    ):
                        param.pose[:, t + 1] = param.pose[:, t]
                    elif (
                        disp[0] < 0
                        and np.abs(disp[1]) < box_w
                        and -disp[0] >= box_l + radius
                        and -disp[0] <= box_l + radius + DSAFE
                    ):
                        param.pose[:, t + 1] = plan.params["pr2"].pose[
                            :, t + 1
                        ] + np.array([0, box_l + radius + 2 * dsafe])
                    elif (
                        disp[0] >= 0
                        and np.abs(disp[1]) < box_w
                        and disp[0] >= box_l + radius
                        ...
                        disp[1] < 0
                        and np.abs(disp[1]) < box_l
                        and -disp[1] >= box_w + radius
                        and -disp[1] <= box_w + radius + DSAFE
                    ):
                        param.pose[:, t + 1] = plan.params["pr2"].pose[
                            :, t + 1
                        ] + np.array([0, box_w + radius + 2 * dsafe])
                    elif (
                        disp[1] >= 0
                        and np.abs(disp[1]) < box_l
                        and disp[1] >= box_w + radius
            ...
            no_col = True
            if check_col:
                dist, rays = self.dist_obs(
                    plan, t, 8, objects=["obs0"], ignore=ignore, return_rays=True
                )
                if np.any(np.abs(dist) < plan.params["pr2"].geom.radius - dtol):
                    ...
                    if old_in_gripper is not None:
                        self._in_gripper = self.in_gripper.name
                    for pname, aname in self.state_inds:
                        if plan.params[pname].is_symbol():
                            continue
                        info[pname, aname] = getattr(plan.params[pname], aname)[:, t]
                        getattr(plan.params[pname], aname)[:, t + 1] = old_state[
            ...
            if no_col:
                for pname, aname in self.state_inds:
                    if plan.params[pname].is_symbol():
                        continue
                    getattr(plan.params[pname], aname)[:, t + 1] = getattr(
            ...
            for pname, aname in self.state_inds:
                if plan.params[pname].is_symbol():
                    continue
                getattr(plan.params[pname], aname)[:, t] = old_state[
                    self.state_inds[pname, aname]
                ]
    ...
    def retime_sample(self, sample):
        ...
        for t in range(sample.T):
            ...
            while (
                np.linalg.norm(u[self.action_inds["pr2", "pose"]]) < 0.05
                and cur_t < sample.T
            ):
            ...
            if cur_t - 1 >= sample.T:
                break
            u[self.action_inds["pr2", "gripper"]] = sample.get(ACTION_ENUM, cur_t - 1)[
                self.action_inds["pr2", "gripper"]
            ]
    ...
    def fill_sample(
        ...
        if OBJ_ENUM in prim_choices:
            ...
            if self.task_list[task[0]].find("move") >= 0:
                ...
                sample.set(
                    ABS_POSE_ENUM, mp_state[self.state_inds[obj_name, "pose"]], t
                )
        ...
        if TARG_ENUM in prim_choices:
            ...
            if self.task_list[task[0]].find("transfer") >= 0:
                ...
                sample.set(
                    ABS_POSE_ENUM, targets[self.target_inds[targ_name, "value"]], t
                )
        ...
        if ABS_POSE_ENUM in prim_choices:
            ...
            if ind < len(task) and type(task[ind]) is not int:
                ...
                sample.set(
                    END_POSE_ENUM,
                    task[ind] - mp_state[self.state_inds["pr2", "pose"]] + grasp,
                    t,
                )
        ...
        for i, obj in enumerate(prim_choices[OBJ_ENUM]):
            ...
            sample.set(
                OBJ_DELTA_ENUMS[i], mp_state[self.state_inds[obj, "pose"]] - ee_pose, t
            )
            sample.set(TARG_ENUMS[i], targ, t)
            sample.set(
                TARG_DELTA_ENUMS[i], targ - mp_state[self.state_inds[obj, "pose"]], t
            )
        ...
        if fill_obs:
            ...
            if (
                ...
                self.reset_mjc_env(
                    sample.get_X(t=t), targets=targets, draw_targets=True
                )
    ...
    def fill_action(self, sample):
        ...
        act[:-1, self.action_inds["pr2", "gripper"]] = X[
            1:, self.action_inds["pr2", "gripper"]
        ]
        act[np.abs(act) > 2] = 2.0
    ...
    def set_grasp(self, grasp, rot):
        ...
        theta = rot * (2.0 * np.pi) / self._hyperparams["sensor_dims"][GRASP_ENUM]
        mat = np.array(
        ...
        for param_name, attr in self.state_inds:
            if attr == "pose":
                ...
                self.mjc_env.set_item_pos(
                    param_name, np.r_[pos, 0.5], mujoco_frame=False, forward=False
                )
                if param_name != "pr2":
                    targ = targets[
                        self.target_inds["{0}_end_target".format(param_name), "value"]
                    ]
                    pos = np.r_[targ, 1.5] if draw_targets else [0, 0, -1]
                    self.mjc_env.set_item_pos(
                        "{0}_end_target".format(param_name), pos, forward=False
                    )
    ...
    def dummy_transition(self, X, task, cond):
        ...
        if task_name.find("movetograsp") > -1:
            new_x[self.state_inds["pr2", "pose"]] = X[self.state_inds[obj, "pose"]] + [
                0,
                -0.75,
            ]
        elif task_name.find("transfer") > -1:
            new_x[self.state_inds["pr2", "pose"]] = self.targets[cond][targ] + [
                0,
                -0.65,
            ]
    ...
    def replace_cond(self, cond, curric_step=-1):
        ...
        if self.master_config["easy"]:
            self.init_vecs[cond][self.state_inds["pr2", "pose"]] = [0, -2.0]
            for pname, aname in self.state_inds:
                inds = self.state_inds[pname, aname]
        ...
        if OBJ_ENUM in prim_choices and curric_step > 0:
            ...
            if curric_step % 2 and step <= len(prim_choices[OBJ_ENUM]):
                ...
                self.x0[cond][self.state_inds["pr2", "pose"]] = (
                    self.x0[cond][
                        self.state_inds[
                            "can{0}".format(inds[len(prim_choices[OBJ_ENUM]) - step]),
                            "pose",
                        ]
                    ]
                    + grasp
    ...
    def permute_hl_data(self, hl_mu, hl_obs, hl_wt, hl_prc, aux, x):
        ...
        for t in range(0, len(hl_mu), nperm):
            ...
            rev_order = [order.tolist().index(n) for n in range(no)]
            cur_inds = np.array(
                [self.state_inds["can{0}".format(n), "pose"] for n in range(no)]
            )
    ...
    def permute_tasks(self, tasks, targets, plan=None, x=None):
        ...
        for n in range(no):
            ...
            if plan is None:
                ...
                old_values["can{0}".format(n)] = plan.params[
                    "can{0}".format(n)
                ].pose.copy()
        perm_map = {}
        for n in range(no):
    ...
    def encode_plan(self, plan, permute=False):
        ...
        for i, l in enumerate(encoded[:-1]):
            if (
                self.task_list[l[0]].find("moveto") >= 0
                and self.task_list[encoded[i + 1][0]].find("transfer") >= 0
            ):
                l[2] = encoded[i + 1][2]
        encoded = [tuple(l) for l in encoded]
        return encoded
    ...
    def encode_action(self, action):
        ...
        for enum in prim_choices:
            if enum is TASK_ENUM or np.isscalar(prim_choices[enum]):
                continue
            l.append(0)
            if hasattr(prim_choices[enum], "__len__"):
                ...
                if self.task_list[l[0]].find("move") >= 0:
                    ...
                    self.task_list[l[0]].find("place") >= 0
                    or self.task_list[l[0]].find("transfer") >= 0
                ):
