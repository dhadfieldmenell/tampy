...
class NAMODoorAgent(NAMOGripAgent):
            ...
            items.append(
                {
                    ...
                    "is_fixed": True,
                    "pos": (0, 0, 1.5),
                    "dimensions": (NEAR_TOL, NEAR_TOL, 0.05),
                    "rgba": tuple(cur_color),
                    "mass": 1.0,
                }
            )
    ...
    def fill_sample(
        ...
        handle_pos = mp_state[self.state_inds["door", "pose"]] + [
            1.5 * np.sin(doortheta),
            1.5 * np.cos(doortheta),
        ]
        ...
        if TARG_ENUM in prim_choices:
            ...
            targ_vec[task[2]] = 1.0
            if (
                self.task_list[task[0]].find("door") >= 0
                or self.task_list[task[0]].find("move") >= 0
            ):
            ...
            sample.targ = task[2]
            if (
                self.task_list[task[0]].find("put") >= 0
                or self.task_list[task[0]].find("leave") >= 0
            ):
        ...
        if ABS_POSE_ENUM in prim_choices:
            ...
            if ind < len(task) and not np.isscalar(task[ind]):
                ...
                sample.set(
                    END_POSE_ENUM,
                    rot.dot(task[ind] - mp_state[self.state_inds["pr2", "pose"]]),
                    t,
                )
        ...
        self.mjc_env.set_joints(
            {
                ...
                "robot_theta": theta,
                "door_hinge": door_theta,
            },
            forward=False,
        )
        for param_name, attr in self.state_inds:
            if param_name == "pr2" or param_name == "door":
                continue
            if attr == "pose":
                pos = mp_state[self.state_inds[param_name, "pose"]].copy()
                targ = self.target_vecs[0][
                ...
                self.mjc_env.set_item_pos(
                    "{0}_end_target".format(param_name), np.r_[targ, 1.5], forward=False
                )
    ...
    def goal_f(
        ...
        door_cost = 0.0 if state[0][self.state_inds["door", "theta"]][0] > -0.3 else 1.0
        cost += door_cost
        if cont:
            return door_cost + alldisp / float(no)
