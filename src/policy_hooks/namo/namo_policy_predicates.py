...
class NAMOPolicyPredicate(ExprPredicate):
    def __init__(
        ...
        super(NAMOPolicyPredicate, self).__init__(
            name,
            e,
            self.attr_inds,
            [],
    ...
    def error_grad(self, x):
        ...
        jac[
            self.action_inds[("pr2", "pose")],
            self.action_inds[("pr2", "pose")] + (active_len - 1) * self.dU,
        ] = -1.0
        jac[
            self.action_inds[("pr2", "gripper")],
            self.action_inds[("pr2", "gripper")] + (active_len - 1) * self.dU,
        ] = -1.0
