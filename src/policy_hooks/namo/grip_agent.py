...
class NAMOGripAgent(NAMOSortingAgent):
    ...
    def run_policy_step(self, u, x):
        ...
        for pname, aname in self.state_inds:
            if aname != "pose":
                continue
            if pname.find("box") < 0 or pname.find("can") < 0:
                continue
        ...
        if ("pr2", "pose") not in self.action_inds:
            ...
            if LOCAL_FRAME:
                cmd_x, cmd_y = np.array(
                    [
                        [np.cos(cur_theta), -np.sin(cur_theta)],
                        [np.sin(cur_theta), np.cos(cur_theta)],
                    ]
                ).dot([rel_x, rel_y])

        if np.isnan(cmd_x):
            cmd_x = 0  # np.random.normal(0.05)
        if np.isnan(cmd_y):
            cmd_y = 0  # np.random.normal(0.05)
        if np.isnan(cmd_theta):
            cmd_theta = 0  # np.random.normal(0.05)
    ...
    def fill_sample(
        ...
        if OBJ_ENUM in prim_choices:
            ...
            if self.task_list[task[0]].find("move") >= 0:
                ...
                sample.set(
                    ABS_POSE_ENUM, mp_state[self.state_inds[obj_name, "pose"]].copy(), t
                )
        ...
        if TARG_ENUM in prim_choices:
            ...
            sample.targ = task[2]
            if (
                self.task_list[task[0]].find("place") >= 0
                or self.task_list[task[0]].find("transfer") >= 0
            ):
                sample.set(END_POSE_ENUM, targ_pose, t)
                sample.set(REL_POSE_ENUM, base_pos, t)
                sample.set(
                    ABS_POSE_ENUM,
                    targets[self.target_inds[targ_name, "value"]].copy(),
                    t,
                )
        ...
        if fill_obs:
            ...
            if (
                ...
                or IM_ENUM in self._hyperparams["cont_obs_include"]
            ):
    ...
    def get_annotated_image(self, s, t, cam_id=None):
        ...
        textover1 = self.mjc_env.get_text_overlay(
            body="Task: {0} {1}".format(task, act)
        )
        textover2 = self.mjc_env.get_text_overlay(
            body="{0: <6} {1: <6} Error: {2}".format(precost, postcost, offset),
    ...
    def fill_cont(self, policy, sample, t):
        ...
        for ind, enum in enumerate(self.continuous_opts):
            ...
            if enum is utils.ABS_POSE_ENUM:
                ...
                sample.set(
                    utils.END_POSE_ENUM,
                    rot.dot(vals[ind] - mp_state[self.state_inds["pr2", "pose"]]),
                    t,
                )
