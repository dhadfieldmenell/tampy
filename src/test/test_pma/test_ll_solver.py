...
class TestLLSolver(unittest.TestCase):
    def setUp(self):
        ...
        self.move_no_obs = get_plan(
            "../domains/namo_domain/namo_probs/move_no_obs.prob"
        )
        ...
        self.move_grasp_moveholding = get_plan(
            "../domains/namo_domain/namo_probs/moveholding.prob"
        )
        ...
        self.putaway2 = get_plan(
            "../domains/namo_domain/namo_probs/putaway2.prob",
            [
    ...
    def test_llparam(self):
        ...
        obj = grb.QuadExpr()
        obj += (
            pr2_ll.pose[0, 0] * pr2_ll.pose[0, 0]
            + pr2_ll.pose[1, 0] * pr2_ll.pose[1, 0]
        )
        model.setObjective(obj)
        model.optimize()
        self.assertTrue(np.allclose(pr2_ll.pose[0, 0].X, 0.0))
        self.assertTrue(np.allclose(pr2_ll.pose[1, 0].X, 0.0))
        ...
        obj = grb.QuadExpr()
        obj += (
            pr2_ll.pose[0, 1] * pr2_ll.pose[0, 1]
            + pr2_ll.pose[1, 1] * pr2_ll.pose[1, 1]
            - 2 * pr2_ll.pose[1, 1]
        )
        model.setObjective(obj)
        model.optimize()
        ...
        self.assertTrue(np.allclose(pr2_ll.pose[0, 1].X, 0.0))
        self.assertTrue(np.allclose(pr2_ll.pose[1, 1].X, 1.0))
        pr2_ll.update_param()
        self.assertTrue(np.allclose(pr2.pose[0, 1], 0.0))
        self.assertTrue(np.allclose(pr2.pose[1, 1], 1.0))

        # robot_init_pose is a Symbol parameter
        model = grb.Model()
        model.params.OutputFlag = 0  # silences Gurobi output
        robot_init_pose_ll = ll_solver.LLParam(
            model, robot_init_pose, horizon, (0, horizon - 1)
        )
