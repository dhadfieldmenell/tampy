...
class TestPlanHDF5Serialization(unittest.TestCase):
    ...
    def test_serialization(self):
        ...
        for param in test_plan.params:
            ...
            self.assertEqual(
                test_plan.params[param].get_type(), self.plan.params[param].get_type()
            )
        ...
        for i in range(len(self.plan.actions)):
            ...
            self.assertEqual(
                test_plan.actions[i].active_timesteps,
                self.plan.actions[i].active_timesteps,
            )
            self.assertEqual(
                test_plan.actions[i].step_num, self.plan.actions[i].step_num
            )
        ...
        for t in range(test_plan.horizon):
            ...
            for i in range(len(active_preds)):
                ...
                self.assertEqual(
                    active_preds[i].get_type(), test_plan_active_preds[i].get_type()
                )
