...
class TestAction(unittest.TestCase):
    ...
    def test_get_failed_preds(self):
        ...
        pred_dict = [{'pred': self.pred0, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (0, 0)},
                     {'pred': self.pred1, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (0, 0)},
        ...
        self.robot.pose = np.array([[2, 2, 7],
                               [5, 5, 9]])
        self.can1.pose = np.array([[3,3,9],
                             [4,4,7]])
        self.can2.pose = np.array([[9,3,6],
                             [4,6,7]])
        self.rpose.value = np.array([[2],
                                [5]])
    ...
    def test_first_failed_ts(self):
        ...
        pred_dict = [{'pred': self.pred0, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (0, 1)},
                     {'pred': self.pred1, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (1, 3)},
                     {'pred': self.pred2, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (2, 3)}]
    ...
    def test_get_active_pred(self):
        ...
        pred_dict = [{'pred': self.pred0, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (0, 5)},
                     {'pred': self.pred1, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (4, 7)},
                     {'pred': self.pred2, 'negated': False, 'hl_info': 'pre', 'active_timesteps': (3, 9)}]
        act = action.Action(4, 'test_action', (0,10), params, pred_dict)
    ...
    def test_copy(self):
        ...
        act = action.Action(4, 'test_action', (1,10), params, pred_dict)
        param_copy_dict = {p: p.copy_ts((1,10)) for p in params}
        act_copy = act.copy(0, param_copy_dict)
        self.assertTrue(act_copy.step_num == act.step_num)
        self.assertTrue(act_copy.name == act.name)
        self.assertTrue(act_copy.active_timesteps == (0,9))
        for p in params:
        ...
        for pred_copy_dict in act_copy.preds:
            ...
            if pred.name == "At_0": # pred0
                ...
                self.assertTrue(pred_copy_dict['active_timesteps'] == (0,4))
            if pred.name == "At_1": # pred1
                self.assertFalse(pred == pred_dict[1])
                self.assertTrue(pred_copy_dict['negated'] == True)
                self.assertTrue(pred_copy_dict['hl_info'] == 'pre')
                self.assertTrue(pred_copy_dict['active_timesteps'] == (3,6))
