...
if USE_TF:
    ...
    def _get_arm_jac(robot_name, njnts):
        ...
        axes = [tf.placeholder(float, (3,), '{}_axis_{}'.format(robot_name, n)) for n in range(njnts)]
        jnt_pos = [tf.placeholder(float, (3,), '{}_jnt_{}'.format(robot_name, n)) for n in range(njnts)]
        ...
        # Rot Jacobian
        arm_jac.extend([tf.dot(obj_dir, tf.cross(axes[n], sign*world_dir)) for n in range(njnts)])
        ...
        tf_cache['{}_arm_jac_grad'.format(robot_name)] = tf.gradients(tf.stack(arm_jac), tf.stack(jnt_pos))
        tf_cache['{}_pos_grad'.format(robot_name)] = tf.gradients(tf.stack(arm_jac), pos)
        tf_cache['{}_dir_grad'.format(robot_name)] = tf.gradients(tf.stack(arm_jac), obj_dir)
        tf_cache['{}_world_grad'.format(robot_name)] = tf.gradients(tf.stack(arm_jac), world_dir)
...
def init_robot_pred(pred, robot, params=[], robot_poses=[], attrs={}):
    ...
    if robot not in attrs:
        attrs[robot] = ['pose'] + r_geom.arms + [r_geom.ee_link_names[arm] for arm in r_geom.arms] + r_geom.ee_attrs
...
class RobotPredicate(ExprPredicate):
    ...
    def set_robot_poses(self, x, robot_body):
        ...
        if len(dof_value_map.keys()): robot_body.set_dof(dof_value_map)
    ...
    def robot_obj_kinematics(self, x):
        ...
        if (self.robot, self.arm) not in self.attr_inds and \
           (self.robot, ee_attr) in self.attr_inds:
...
class InGripper(PosePredicate):
    ...
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        ...
        attr_inds, attr_dim = init_robot_pred(self, self.robot, [self.obj], attrs={self.robot: ee_attrs})
        self._param_to_body = {self.robot: self.lazy_spawn_or_body(self.robot, self.robot.name, self.robot.geom),
