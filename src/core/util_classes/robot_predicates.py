...
class RobotPredicate(ExprPredicate):
    ...
    def setup_mov_limit_check(self, delta=False, ee_only=False):
        ...
        if ee_only:
            ...
            lb = np.zeros(
                3
                + sum([len(dof_map[arm]) for arm in geom.arms])
...
class InContact(ExprPredicate):
    ...
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        ...
        attr_inds, attr_dim = init_robot_pred(
            self, params[0], [], attrs={params[0]: [self.gripper]}
        )
...
class CloseGripper(InContact):
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        self.GRIPPER_CLOSE = np.array(
            params[0].geom.get_gripper_closed_val(scalar=False)
        )  # const.GRIPPER_CLOSE_VALUE
        self.GRIPPER_OPEN = np.array(
            params[0].geom.get_gripper_open_val(scalar=False)
        )  # const.GRIPPER_OPEN_VALUE
    ...
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        ...
        if params[1].is_symbol():
            ...
            self._param_to_body = {
                ...
                self.obj: self.lazy_spawn_or_body(
                    self.obj, self.obj.name, self.obj.geom
                ),
            }
...
class EEValid(PosePredicate):
    ...
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        ...
        if hasattr(self, "dist"):
            self.eval_dim = 12
            e = LEqExpr(
                Expr(self.tile_f, self.tile_grad),
                self.coeff * self.dist * np.ones((self.eval_dim, 1)),
            )
        else:
            self.eval_dim = 6  # 4
            e = EqExpr(Expr(self.eval_f, self.eval_grad), np.zeros((self.eval_dim, 1)))
    ...
    def stacked_f(self, x):
        # return self.coeff * self.pos_check_f(x, self.rel_pt)
        val = np.vstack(
            [self.coeff * self.pos_check_f(x), self.rot_coeff * self.ee_rot_check_f(x)]
        )
        return val
...
class EEReachable(PosePredicate):
    ...
    def stacked_grad(self, x):
        ...
        for s in range(start, end + 1):
            rel_pt = self.rel_pt + self.get_rel_pt(s)
            grad[j : j + dim, i : i + self.attr_dim] = (
                self.coeff
                * self.mask
                * self.rel_ee_pos_check_jac(x[i : i + self.attr_dim], rel_pt)
            )
            j += dim
            # if s == 0:
...
class EEApproachLeft(EEReachable):
    def __init__(
        ...
        self.arm = "left"
        super(EEApproachLeft, self).__init__(
...
class EERetreatLeft(EEReachable):
    def __init__(
        ...
        self.arm = "left"
        super(EERetreatLeft, self).__init__(
...
class EEAtLeftRot(EEReachableRot):
    def __init__(
        ...
        self.arm = "left"
        super(EEReachableLeftRot, self).__init__(
            name, params, expected_param_types, (0, 0), env, debug
        )
...
class EEAtXYLeft(EEReachableLeft):
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        ...
        super(EEAtXYLeft, self).__init__(
            name, params, expected_param_types, env, debug, 0
        )
        self.mask = np.array([1.0, 1.0, 0.0]).reshape((3, 1))
        self.approach_dist = const.GRASP_DIST
...
class EEApproachRight(EEReachable):
    def __init__(
        ...
        self.arm = "right"
        super(EEApproachRight, self).__init__(
            name, params, expected_param_types, (-steps, 0), env, debug
        )
...
class EERetreatRight(EEReachable):
    def __init__(
        ...
        self.arm = "right"
        super(EERetreatRight, self).__init__(
            name, params, expected_param_types, (0, steps), env, debug
        )
...
class EEAtRightRot(EEReachableRot):
    def __init__(
        ...
        self.arm = "right"
        self.coeff = const.NEAR_APPROACH_ROT_COEFF
        super(EEAtRightRot, self).__init__(
            name, params, expected_param_types, (0, 0), env, debug
        )
...
class EEAtXYRight(EEReachableRight):
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        self.coeff = const.EEATXY_COEFF
        super(EEAtXYRight, self).__init__(
            name, params, expected_param_types, env=env, debug=debug, steps=0
        )
        self.mask = np.array([1.0, 1.0, 0.0]).reshape((3, 1))
        self.approach_dist = const.GRASP_DIST
...
class ObstructsHolding(CollisionPredicate):
    ...
    def __init__(
        ...
        attr_inds, attr_dim = init_robot_pred(
            self, self.robot, [self.obstacle, self.obj]
        )
        ...
        if self.obstacle is not self.obj:
            attr_inds, attr_dim = init_robot_pred(
                self, self.robot, [self.obstacle, self.obj], attrs=attrs
            )
        else:
            attr_inds, attr_dim = init_robot_pred(
                self, self.robot, [self.obj], attrs=attrs
            )
        self._param_to_body = {
            self.robot: self.lazy_spawn_or_body(
                self.robot, self.robot.name, self.robot.geom
...
class HeightBlock(ExprPredicate):
    def __init__(self, name, params, expected_param_types, env=None, debug=False):
        ...
        attr_inds = OrderedDict(
            [
                (self.goal_obj, [("pose", np.array([0, 1, 2], dtype=np.int))]),
        ...
        super(HeightBlock, self).__init__(
            name,
            e,
            attr_inds,
            params,
...
class AboveTable(ExprPredicate):
    def __init__(self, name, params, expected_param_types, env=None):
        ...
        super(AboveTable, self).__init__(
            name, e, attr_inds, params, expected_param_types, priority=-2
        )
        self.spacial_anchor = True
