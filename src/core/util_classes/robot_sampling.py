...
from sco_gurobi.expr import Expr
import math
import numpy as np
...
def resample_gripper_down_rot(pred, negated, t, plan, arms=[]):
    ...
    iks = {}
    robot.openrave_body.set_dof(
        {pred.arm: np.zeros(len(robot.geom.jnt_names[pred.arm]))}
    )
    for arm in arms:
    ...
    add_to_attr_inds_and_res(
        t, attr_inds, res, robot, [(arm, iks[arm]) for arm in arms]
    )
    return res, attr_inds
...
def resample_eereachable(pred, negated, t, plan, inv=False):
    ...
    acts = [
        ...
        if a.active_timesteps[0] < t and a.active_timesteps[1] >= t
    ]
    if not len(acts):
        return None, None
    act = acts[0]
    ...
    for ts in range(max(a_st, t + st), min(a_et - 1, t + et)):
        ...
        add_to_attr_inds_and_res(
            ts, attr_inds, res, robot, [(arm, np.array(ik).flatten())]
        )
    return res, attr_inds
