...
Types: Basket, BasketTarget, RobotPose, Robot, EEPose, Obstacle, Washer, WasherPose, Cloth, ClothTarget, Rotation
...
class PutIntoBasket(Action):
    def __init__(self):
        ...
        self.eff = [\
            ...
            ('(not (BaxterRobotAt ?robot ?sp))', '{}:{}'.format(end, end-1)),
            ('(BaxterClothTargetInBasket ?ct ?bt)', '{}:{}'.format(end-1, end-1)),
            ('(BaxterRobotAt ?robot ?ep)', '{}:{}'.format(end, end)),
            ('(not (BaxterClothInGripperLeft ?robot ?cloth))', '{}:{}'.format(end, end)),
            ('(forall (?sym1 - RobotPose)\
                (forall (?sym2 - RobotPose)\
...
print(dom_str)
f = open('laundry_policy.domain', 'w')
f.write(dom_str)
