...
def main():
    for NUM_CANS in range(1, 6):
        for N_END in range(1, 10):
            for N_AUX in range(0, 5):
                filename = "namo_probs/door_prob_{0}_{1}end_{2}aux.prob".format(NUM_CANS, N_END, N_AUX)
                ...
                s += "(geom %s %s %s), (pose %s %s), "%("door", 0.3, 1.5, "door", [-1., 3.])
                s += "(gripper pr2 [-0.1]), "
                s += "(theta pr2 [0.]), "
                s += "(theta door [0.]), "
                s += "(vel pr2 [0.]), "
