...
class OpenDoor(Action):
    def __init__(self):
        ...
        self.pre = [\
                ...
                ('(DoorClosed ?door)', '1:{0}'.format(et-6)),
                ('(forall (?gr - Grasp) (forall (?obj - Can) (not (NearGraspAngle ?robot ?obj))))', '0:0'),
                ('(not (GripperClosed ?robot))', '1:{0}'.format(et-7)),
                ('(GripperClosed ?robot)', '{0}:{1}'.format(et-6, et-2)),
                ('(not (GripperClosed ?robot))', '{0}:{1}'.format(et-1, et-1)),
                ('(InDoorAngle ?robot ?door)', '{0}:{1}'.format(et-8, et-6)),
                ('(DoorInGrasp ?robot ?door)', '{0}:{1}'.format(et-6, et-2)),
                ...
                ('(forall (?w - Obstacle) (forall (?obj - Can) (not (Collides ?obj ?w))))', '1:{0}'.format(et-1)),
                ('(forall (?w - Obstacle) (not (RCollides ?robot ?w)))', '1:{0}'.format(et-8)),
                ('(forall (?obj - Can) (not (Obstructs ?robot ?sp ?gp ?obj)))', '0:0'),
                ('(forall (?obj - Can) (not (WideObstructs ?robot ?sp ?gp ?obj)))', '0:-1'),
                ('(forall (?obj - Can) (not (WideObstructs ?robot ?sp ?gp ?obj)))', '1:{0}'.format(et-4)),
                ('(forall (?obj - Can) (not (Obstructs ?robot ?sp ?gp ?obj)))', '{0}:{1}'.format(et-3, et-3)),
                ('(ThetaDirValid ?robot)', '{0}:{1}'.format(1, et-8)),
                ('(ForThetaDirValid ?robot)', '{0}:{1}'.format(et-8, et-7)),
                #('(ThetaDirValid ?robot)', '{0}:{1}'.format(et-7, et-1)),
        ]
        self.eff = [\
                ('(not (DoorClosed ?door))', '{0}:{1}'.format(et-1, et)),
...
class CloseDoor(Action):
    def __init__(self):
        ...
        self.pre = [\
                ...
                ('(not (DoorClosed ?door))', '1:{0}'.format(et-6)),
                ('(forall (?gr - Grasp) (forall (?obj - Can) (not (NearGraspAngle ?robot ?obj))))', '0:0'),
                ('(not (GripperClosed ?robot))', '1:{0}'.format(et-7)),
                ('(GripperClosed ?robot)', '{0}:{1}'.format(et-6, et-2)),
                ('(not (GripperClosed ?robot))', '{0}:{1}'.format(et-1, et-1)),
                ('(InDoorAngle ?robot ?door)', '{0}:{1}'.format(et-8, et-1)),
                ('(DoorInGrasp ?robot ?door)', '{0}:{1}'.format(et-7, et-2)),
                ('(forall (?obj - Can) (Stationary ?obj))', '0:{0}'.format(et-1)),
                ('(StationaryDoorPos ?door)', '0:{0}'.format(et-1)),
                ('(forall (?w - Obstacle) (StationaryW ?w))', '0:{0}'.format(et-1)),
                ('(IsMP ?robot)', '0:{0}'.format(et-1)),
        ...
        self.eff = [\
                ('(DoorClosed ?door)', '{0}:{1}'.format(et-1, et)),
...
actions = [MoveTo(), Transfer(), Place(), OpenDoor(), CloseDoor()]
