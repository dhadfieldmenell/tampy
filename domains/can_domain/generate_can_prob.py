...
def main():
    ...
    for iteration in range(NUM_PROBS):
        ...
        coords = list(itertools.product(range(LENGTH), range(HEIGHT), range(WIDTH)))
        ...
        for i in range(NUM_TARGETS):
            s += "(IsPDP pdp_target%d location%d), "%(i, i)
        s += "(RobotAt pr2 robot_init_pose)\n\n"

        s += "Goal: %s"%GOAL

        with open("can_probs/can_%d_%d.prob"%(SEED, iteration), "w") as f:
            f.write(s)
